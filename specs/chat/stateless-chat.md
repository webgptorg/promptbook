...
4. **Credentials** — per-user/per-agent secrets are resolved and injected as prompt parameters: GitHub token when the source declares `USE PROJECT` ([GitHub projects](../integrations/github-projects.md)), Google access token when it declares `USE CALENDAR` ([Calendar](../integrations/calendar.md)), and the SMTP credential for [email](../integrations/email.md) — each falling back to [wallet](../users/wallet.md) records.
