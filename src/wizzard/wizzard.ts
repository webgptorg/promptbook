...
class Wizzard {
	...
	> {
		if (!$isRunningInNode()) {
			throw new EnvironmentMismatchError(
				"Wizzard works only in Node.js environment",
			);
		}
		...
		if (outputParametersLength === 0) {
			...
			resultString =
				outputParameters[Object.keys(outputParameters)[0]!] ||
				VALUE_STRINGS.undefined;
		} else {
	...
	private executionTools: Required<
		Pick<ExecutionTools, "fs" | "fetch">
	> | null = null;
	 ...
	 */
	public async getExecutionTools(): Promise<
		Required<Pick<ExecutionTools, "fs" | "fetch">>
	> {
