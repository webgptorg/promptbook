...
export function executionReportJsonToString(
	...
	if (executionReportJson.promptExecutions.length !== 0) {
		...
		const startedAt = moment(
			Math.min(
				...executionReportJson.promptExecutions
					...
					.map((promptExecution) =>
						moment(promptExecution.result!.timing.start).valueOf(),
					),
			),
		);
		const completedAt = moment(
			Math.max(
				...executionReportJson.promptExecutions
					...
					.map((promptExecution) =>
						moment(promptExecution.result!.timing.complete).valueOf(),
					),
			),
		);

		const timingItems: FromtoItems = executionReportJson.promptExecutions.map(
			(promptExecution) => ({
		...
		const costItems: FromtoItems = executionReportJson.promptExecutions
			.filter(
				(promptExecution) =>
					typeof promptExecution.result?.usage?.price === "number",
			)
			.map((promptExecution) => ({
				title: promptExecution.prompt.title,
				from: 0,
				to:
					(promptExecution.result?.usage?.price.value ||
						0) /* <- TODO: look at uncertain numbers */ *
					(1 + taxRate),
			}));

		const duration = moment.duration(completedAt.diff(startedAt));
		const llmDuration = moment.duration(
			countWorkingDuration(timingItems) * 1000,
		);

		const executionsWithKnownCost = executionReportJson.promptExecutions.filter(
			(promptExecution) =>
				(promptExecution.result?.usage?.price || "UNKNOWN") !== "UNKNOWN",
		);
		const cost: number_usd = executionsWithKnownCost.reduce(
			(cost, promptExecution) =>
				cost +
				(promptExecution.result!.usage.price
					.value /* <- Look at uncertain number */ || 0),
			0,
		);

		headerList.push(
			`STARTED AT ${moment(startedAt).format(`YYYY-MM-DD HH:mm:ss`)}`,
		);
		headerList.push(
			`COMPLETED AT ${moment(completedAt).format(`YYYY-MM-DD HH:mm:ss`)}`,
		);
		headerList.push(
			`TOTAL DURATION ${duration.humanize(MOMENT_ARG_THRESHOLDS)}`,
		);
		headerList.push(
			`TOTAL LLM DURATION ${llmDuration.humanize(MOMENT_ARG_THRESHOLDS)}`,
		);

		headerList.push(
			`TOTAL COST $${numberToString(cost * (1 + taxRate))}` +
				(executionsWithKnownCost.length ===
				executionReportJson.promptExecutions.length
					? ""
		...
		executionReportString +=
			...
			executionReportJson.promptExecutions
				.map((promptExecution) => {
					...
					if (
						/^\s*\p{Extended_Pictographic}/u.test(promptExecution.prompt.title)
					) {
						hash = "-" + hash;
					}
        ...
        */
			...
			executionReportString +=
				...
				spaceTrim(
					(block) => `
                        ...
                        ${block(
													escapeMarkdownBlock(
														promptExecution.result?.rawPromptContent ||
															promptExecution.prompt.content,
													),
												)}
                        \`\`\`
		...
		if (promptExecution.result && promptExecution.result.content) {
			...
			if (promptExecution.result === undefined) {
				...
				executionReportString += embeddingVectorToString(
					promptExecution.result.content,
				);
			}
		}
