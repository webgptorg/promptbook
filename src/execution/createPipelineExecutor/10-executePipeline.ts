...
export async function executePipeline(
	...
	for (const parameterName of Object.keys(inputParameters)) {
		const parameter = preparedPipeline.parameters.find(
	...
	try {
		let resovedParameterNames: ReadonlyArray<string_name> =
			preparedPipeline.parameters
				.filter(({ isInput }) => isInput)
				.map(({ name }) => name);
		let unresovedTasks: ReadonlyArray<ReadonlyDeep<TaskJson>> = [
			...preparedPipeline.tasks,
		];
		...
		while (unresovedTasks.length > 0) {
			...
			const currentTask = unresovedTasks.find((task) =>
				task.dependentParameterNames.every((name) =>
					[...resovedParameterNames, ...RESERVED_PARAMETER_NAMES].includes(
						name,
					),
				),
			);
		...
		const usage = addUsage(
			...executionReport.promptExecutions.map(
				({ result }) => result?.usage || ZERO_USAGE,
			),
		);
