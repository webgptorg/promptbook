 ...
 */
export type ExecuteAttemptsOptions = Required<
	Omit<CreatePipelineExecutorOptions, "pipeline">
> & {
	/**
	 * @@@
	 */
...
export async function executeAttempts(
	...
	for (
		let attempt = -jokerParameterNames.length;
		attempt < maxAttempts;
		attempt++
	) {
		const isJokerAttempt = attempt < 0;
		const jokerParameterName =
			jokerParameterNames[jokerParameterNames.length + attempt];
			...
			if (!isJokerAttempt) {
				switch (task.taskType) {
							...
							switch (modelRequirements.modelVariant) {
								...
								case "COMPLETION":
									$ongoingTaskResult.$completionResult =
										await llmTools.callCompletionModel!(
					...
					case "SCRIPT_TASK":
						...
						if (
							$ongoingTaskResult.$scriptPipelineExecutionErrors.length === 1
						) {
					...
					case "DIALOG_TASK":
						...
						$ongoingTaskResult.$resultString =
							await tools.userInterface.promptDialog(
								$deepFreeze({
			...
			if (task.format) {
				if (task.format === "JSON") {
					if (!isValidJsonString($ongoingTaskResult.$resultString || "")) {
						...
						try {
							$ongoingTaskResult.$resultString = extractJsonBlock(
								$ongoingTaskResult.$resultString || "",
							);
						} catch (error) {
							keepUnused(
