 ...
 * - The callChatModel method now accepts an optional 'functions' parameter.
 * - If provided, functions are added to the rawRequest sent to OpenAI.
 * - The response may include a function_call, which is extracted and returned in the result.
 */
export abstract class OpenAiCompatibleExecutionTools implements LlmExecutionTools /* <- TODO: [🍚] `, Destroyable` */ {
    ...
    public async callChatModel(
        ...
        const { content, parameters, modelRequirements, format, functions } = prompt;
        ...
        if (functions && functions.length > 0) {
            rawRequest.functions = functions;
        }
        ...
        const functionCall = rawResponse.choices[0].message.function_call;
