 ...
 */
export class OpenAiExecutionTools
	implements LlmExecutionTools /* <- TODO: [🍚] `, Destroyable` */
{
	 ...
	 */
	public createAssistantSubtools(
	...
	public async callChatModel(
		prompt: Pick<
			Prompt,
			"content" | "parameters" | "modelRequirements" | "format"
		>,
		...
		const rawResponse = await client.chat.completions
			.create(rawRequest)
			.catch((error) => {
				if (this.options.isVerbose) {
	...
	public async callCompletionModel(
		...
		const modelName =
			modelRequirements.modelName || this.getDefaultCompletionModel().modelName;
		const modelSettings = {
			model: modelName,
		...
		const rawResponse = await client.completions
			.create(rawRequest)
			.catch((error) => {
				if (this.options.isVerbose) {
					console.info(colors.bgRed("error"), error);
	...
	public async callEmbeddingModel(
		...
		const modelName =
			modelRequirements.modelName || this.getDefaultEmbeddingModel().modelName;
