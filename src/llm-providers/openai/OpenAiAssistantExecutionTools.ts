 ...
 */
export class OpenAiAssistantExecutionTools
	...
	implements LlmExecutionTools
{
	...
	public async callChatModel(
		...
		for (const key of [
			...
			"seed",
			"temperature",
		] as Array<keyof ModelRequirements>) {
			if (modelRequirements[key] !== undefined) {
				throw new NotYetImplementedError(
					`In \`OpenAiAssistantExecutionTools\` you cannot specify \`${key}\``,
				);
			}
		}
        ...
        */
		...
		if (rawResponse.length !== 1) {
			throw new PipelineExecutionError(
				`There is NOT 1 BUT ${rawResponse.length} finalMessages from OpenAI`,
			);
		}
