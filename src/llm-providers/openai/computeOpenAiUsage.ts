...
export function computeOpenAiUsage(
	...
	if (rawResponse.usage === undefined) {
		throw new PipelineExecutionError(
			"The usage is not defined in the response from OpenAI",
		);
	}

	if (rawResponse.usage?.prompt_tokens === undefined) {
		throw new PipelineExecutionError(
			"In OpenAI response `usage.prompt_tokens` not defined",
		);
	}

	const inputTokens = rawResponse.usage.prompt_tokens;
	const outputTokens =
		(rawResponse as OpenAI.Chat.Completions.ChatCompletion).usage
			?.completion_tokens || 0;

	const modelInfo = OPENAI_MODELS.find(
