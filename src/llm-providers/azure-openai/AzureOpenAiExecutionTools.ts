 ...
 */
export class AzureOpenAiExecutionTools
	implements LlmExecutionTools /* <- TODO: [🍚] `, Destroyable` */
{
	...
	public async callChatModel(
		...
		try {
			const modelName =
				prompt.modelRequirements.modelName || this.options.deploymentName;
			const modelSettings = {
			...
			const rawResponse = await this.withTimeout(
				client.getChatCompletions(...rawRequest),
			).catch((error) => {
				if (this.options.isVerbose) {
			...
			if (rawResponse.choices.length > 1) {
				...
				throw new PipelineExecutionError(
					"More than one choise from Azure OpenAI",
				);
			}
			...
			throw this.transformAzureError(
	...
	public async callCompletionModel(
		...
		if (modelRequirements.modelVariant !== "COMPLETION") {
			throw new PipelineExecutionError(
				"Use callCompletionModel only for COMPLETION variant",
			);
		}
		...
		try {
			...
			if (this.options.isVerbose) {
				console.info(
					colors.bgWhite("content"),
					JSON.stringify(content, null, 4),
				);
				console.info(
					colors.bgWhite("parameters"),
			...
			const rawResponse = await this.withTimeout(
				client.getCompletions(...rawRequest),
			).catch((error) => {
				if (this.options.isVerbose) {
					console.info(colors.bgRed("error"), error);
				}
	 ...
	 */
	private transformAzureError(azureError: {
		code: string;
		message: string;
	}): Error {
		...
		return new PipelineExecutionError(
			`${code || "(No Azure error code)"}: ${message}`,
		);
	}
}
