 ...
 */
export class RemoteLlmExecutionTools<TCustomOptions = undefined>
	implements LlmExecutionTools
{
	...
	public async listModels(): Promise<ReadonlyArray<AvailableModel>> {
		...
		const promptResult = await new Promise<ReadonlyArray<AvailableModel>>(
			(resolve, reject) => {
				socket.on(
					"listModels-response",
					(response: PromptbookServer_ListModels_Response) => {
	...
	public callChatModel(prompt: ChatPrompt): Promise<ChatPromptResult> {
		...
		return /* not await */ this.callCommonModel(
			prompt,
		) as Promise<ChatPromptResult>;
	}

	/**
	 * Calls remote proxy server to use a completion model
	 */
	public callCompletionModel(
		...
		return /* not await */ this.callCommonModel(
			prompt,
		) as Promise<CompletionPromptResult>;
	}

	/**
	 * Calls remote proxy server to use a embedding model
	 */
	public callEmbeddingModel(
		...
		return /* not await */ this.callCommonModel(
			prompt,
		) as Promise<EmbeddingPromptResult>;
	}
	...
	private async callCommonModel(prompt: Prompt): Promise<PromptResult> {
		...
		const promptResult = await new Promise<PromptResult>((resolve, reject) => {
			socket.on(
				"prompt-response",
				(response: PromptbookServer_Prompt_Response) => {
