...
async function playground() {
	...
	console.info(
		colors.bgCyan("Playground:"),
		colors.bgWhite(`Starting remote server`),
	);
	startRemoteServer({
		rootPath: "/promptbook",
		port: 4460,
		...
		createLlmExecutionTools(options) {
			...
			console.info(colors.bgCyan("Playground:"), {
				appId,
				userId,
				customOptions,
			});
	...
	for (const mode of ["anonymous", "collection"] as const) {
		...
		console.info(
			colors.bgCyan("Playground:"),
			colors.bgWhite(`Creating RemoteLlmExecutionTools (${mode} mode) `),
		);
		...
		console.info(
			colors.bgCyan("Playground:"),
			colors.bgWhite(`Checking configuration...`),
		);
		await tools.checkConfiguration();
		console.info(
			colors.bgCyan("Playground:"),
			colors.bgGreen(`Configuration checked!`),
		);

		await forTime(500);
		console.info(
			colors.bgCyan("Playground:"),
			colors.bgWhite(`Listing models...`),
		);
		const models = await tools.listModels();
		console.info({ models });
		console.info(
			colors.bgCyan("Playground:"),
			colors.bgGreen(`Models listed!`),
		);
	}
