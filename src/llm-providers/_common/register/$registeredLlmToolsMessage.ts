...
export function $registeredLlmToolsMessage(): string_markdown {
	 ...
	 */
	const all: Array<
		Registered & Partial<Pick<LlmToolsMetadata, "title" | "envVariables">>
	> = [];
	...
	for (const {
		...
		className,
		envVariables,
	} of $llmToolsMetadataRegister.list()) {
		if (
			all.some(
				(item) =>
