...
export const sectionCommandParser: PipelineTaskCommandParser<SectionCommand> = {
	...
	$applyToTaskJson(
		...
		const expectResultingParameterName = () => {
			...
			throw new ParseError(
				`Task section and example section must end with return statement -> {parameterName}`,
			);
		};
		...
		if (command.taskType === "EXAMPLE") {
			...
			const parameter = $pipelineJson.parameters.find(
				(param) => param.name === $taskJson.resultingParameterName,
			);
		...
		if (command.taskType === "INSTRUMENT") {
			console.error(
				new NotYetImplementedError("Instruments are not implemented yet"),
			);
