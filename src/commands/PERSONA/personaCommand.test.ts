...
describe("how PERSONA command in .book.md files works", () => {
	...
	it("should parse PERSONA with description", () => {
		expect(
			parseCommand(
				"PERSONA John, male 38 years old programmer",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PERSONA",
		...
		expect(
			parseCommand(
				"PERSONA Joe, male 38 years old programmer",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PERSONA",
		...
		expect(
			parseCommand(
				"PERSONA Jiří, male 38 years old programmer",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PERSONA",
		...
		expect(
			parseCommand(
				"PERSONA Іван, male 38 years old programmer",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PERSONA",
		...
		expect(
			parseCommand(
				"PERSONA 田中, male 38 years old programmer",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PERSONA",
		...
		expect(
			parseCommand(
				"PERSONA محمد مصدق, male 38 years old programmer",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PERSONA",
	...
	it("should fail parsing PERSONA command", () => {
		expect(() => parseCommand("PERSONA", "PIPELINE_HEAD")).toThrowError(
			/You must set name for the persona/i,
		);
		expect(() => parseCommand("PERSONA ,", "PIPELINE_HEAD")).toThrowError(
			/You must set name for the persona/i,
		);
