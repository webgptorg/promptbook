...
describe("how PARAMETER command in .book.md files works", () => {
	it("should parse PARAMETER command", () => {
		expect(
			parseCommand("parameter {name} Name for the hero1", "PIPELINE_HEAD"),
		).toEqual({
			type: "PARAMETER",
			isInput: false,
			isOutput: false,
		...
		// Note: [🦈] and [🐡]
		expect(
			parseCommand("> {name} Name for the hero3", "PIPELINE_HEAD"),
		).toEqual({
			type: "PARAMETER",
			isInput: false,
			isOutput: false,
		...
		expect(
			parseCommand(
				"input parameter {name} Name for the hero5",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PARAMETER",
		...
		expect(
			parseCommand("input   parameter {name}          ", "PIPELINE_HEAD"),
		).toEqual({
			type: "PARAMETER",
			isInput: true,
			isOutput: false,
		...
		expect(
			parseCommand(
				"OUTPUT parameter {name} Name for the hero6",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PARAMETER",
		...
		expect(
			parseCommand(
				"   parameter    {name}        Name for the hero7         ",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PARAMETER",
	...
	it("should not be confused by input/output word in parameter name or description", () => {
		// Note: [🐡]
		expect(
			parseCommand("> {inputText} The input text", "PIPELINE_HEAD"),
		).toEqual({
			type: "PARAMETER",
		...
		// Note: [🐡]
		expect(
			parseCommand("> {outputText} The output text", "PIPELINE_HEAD"),
		).toEqual({
			type: "PARAMETER",
			isInput: false,
		...
		expect(
			parseCommand("PARAMETER {inputText} The input text", "PIPELINE_HEAD"),
		).toEqual({
			type: "PARAMETER",
			isInput: false, // <- Note: Not input despite the word input in the parameter name
			isOutput: false,
		...
		expect(
			parseCommand("PARAMETER {outputText} The output text", "PIPELINE_HEAD"),
		).toEqual({
			type: "PARAMETER",
			isInput: false,
			isOutput: false, // <- Note: Not output despite the word output in the parameter name
		...
		expect(
			parseCommand(
				"OUTPUT PARAMETER {inputText} The input text",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PARAMETER",
		...
		expect(
			parseCommand(
				"INPUT PARAMETER {outputText} The output text",
				"PIPELINE_HEAD",
			),
		).toEqual({
			type: "PARAMETER",
	...
	it("should fail parsing PARAMETER command", () => {
		expect(() => parseCommand("parameter {}", "PIPELINE_HEAD")).toThrowError(
			/Invalid parameter/i,
		);
		expect(() =>
			parseCommand("parameter { name }", "PIPELINE_HEAD"),
		).toThrowError(/Invalid parameter/i);
		expect(() =>
			parseCommand("parameter {name} {name}", "PIPELINE_HEAD"),
		).toThrowError(/Can not contain another parameter in description/i);
		expect(() =>
			parseCommand(
				"parameter {name} {name} Name for the hero9",
				"PIPELINE_HEAD",
			),
		).toThrowError(/Can not contain another parameter in description/i);
		expect(() =>
			parseCommand(
				"parameter {name} Name for the hero9 {name}",
				"PIPELINE_HEAD",
			),
		).toThrowError(/Can not contain another parameter in description/i);
		expect(() =>
			parseCommand(
				"parameter {name} Name for the hero {name} Name for the herox",
				"PIPELINE_HEAD",
			),
		).toThrowError(/Can not contain another parameter in description/i);
		expect(() =>
			parseCommand("parmeter {name} Name for the heroy", "PIPELINE_HEAD"),
