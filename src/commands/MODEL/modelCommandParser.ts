...
export const modelCommandParser: PipelineBothCommandParser<ModelCommand> = {
	...
	$applyToPipelineJson(
		...
		if ($pipelineJson.defaultModelRequirements[command.key] !== undefined) {
			if (
				...
				console.warn(
					`Multiple commands \`MODEL ${command.key} ${command.value}\` in the pipeline head`,
				);
	...
	$applyToTaskJson(
		...
		if ($taskJson.taskType !== "PROMPT_TASK") {
			throw new ParseError(
				`MODEL command can only be used in PROMPT_TASK block`,
			);
		}
		...
		if (
			command.value ===
			($pipelineJson.defaultModelRequirements || {})[command.key]
		) {
			console.log(
				spaceTrim(`
