...
describe("how MODEL command in .book.md files works", () => {
	it("should parse MODEL command", () => {
		...
		expect(
			parseCommand("MODEL VARIANT Completion   ", "PIPELINE_TASK"),
		).toEqual({
			type: "MODEL",
			key: "modelVariant",
			value: "COMPLETION",
		...
		expect(
			parseCommand("MODEL NAME gpt-4-1106-preview", "PIPELINE_TASK"),
		).toEqual({
			type: "MODEL",
			key: "modelName",
		...
		expect(
			parseCommand("MODEL NAME gpt-3.5-turbo-instruct", "PIPELINE_TASK"),
		).toEqual({
			type: "MODEL",
			key: "modelName",
	...
	it("should fail parsing MODEL VARIANT command", () => {
		expect(() => parseCommand("MODEL wet", "PIPELINE_TASK")).toThrowError(
			/Unknown model key/i,
		);
		expect(() => parseCommand("MODEL {script}", "PIPELINE_TASK")).toThrowError(
			/Unknown model key/i,
		);
	});
