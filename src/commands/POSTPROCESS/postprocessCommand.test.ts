...
describe('how POSTPROCESS command in .book.md files works', () => {
    ...
    it('should fail parsing POSTPROCESS command', () => {
        expect(() => parseCommand('Postprocess spaceTrim unwrapResult', 'PIPELINE_TASK')).toThrowError(
        ...
        expect(() => parseCommand('Process spaceTrim', 'PIPELINE_TASK')).toThrowError(/Unknown command/i);
        expect(() => parseCommand('Postprocess', 'PIPELINE_TASK')).toThrowError(
        ...
        expect(() => parseCommand('POSTPROCESS as^fadf', 'PIPELINE_TASK')).toThrowError(
            /Invalid postprocess function name/i,
        );
    });
