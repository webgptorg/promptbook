...
describe('how EXPECT command in .book.md files works', () => {
    it('should parse EXPECT command', () => {
        ...
        expect(parseCommand('Expect minimally 1 character', 'PIPELINE_TASK')).toEqual({
            type: 'EXPECT',
            sign: 'MINIMUM',
            unit: 'CHARACTERS',
            amount: 1,
        ...
        expect(parseCommand('Expect maximally 5 characters', 'PIPELINE_TASK')).toEqual({
            type: 'EXPECT',
            sign: 'MAXIMUM',
            unit: 'CHARACTERS',
            amount: 5,
        ...
        expect(parseCommand('Expect exactly 2 paragraphs', 'PIPELINE_TASK')).toEqual({
            type: 'EXPECT',
            sign: 'EXACTLY',
            unit: 'PARAGRAPHS',
            amount: 2,
    ...
    it('should fail parsing expect command', () => {
        expect(() => parseCommand('Expect foo 1 char', 'PIPELINE_TASK')).toThrowError(/Invalid EXPECT command/i);
        expect(() => parseCommand('Expect min 1 vars', 'PIPELINE_TASK')).toThrowError(/Invalid EXPECT command/i);
        expect(() => parseCommand('Expect min chars', 'PIPELINE_TASK')).toThrowError(/Invalid EXPECT command/i);
        expect(() => parseCommand('Expect min xx chars', 'PIPELINE_TASK')).toThrowError(/Invalid EXPECT command/i);
        expect(() => parseCommand('Expect exactly 2 p', 'PIPELINE_TASK')).toThrowError(/Ambiguous unit "p"/i);
        expect(() => parseCommand('EXPECT', 'PIPELINE_TASK')).toThrowError(/Invalid EXPECT command/i);
        expect(() => parseCommand('EXPECT brr', 'PIPELINE_TASK')).toThrowError(/Invalid EXPECT command/i);
    });
