		...
		try {
			...
			if (/^exact/i.test(signRaw)) {
				...
				throw new ParseError(
					`Invalid sign "${signRaw}", expected EXACTLY, MIN or MAX`,
				);
			}
			...
			for (const existingUnit of EXPECTATION_UNITS) {
				...
				existingUnitText = existingUnitText.substring(
					0,
					existingUnitText.length - 1,
				);
				...
				if (
					new RegExp(`^${existingUnitText.toLowerCase()}`).test(
						unitRaw.toLowerCase(),
					) ||
					new RegExp(`^${unitRaw.toLowerCase()}`).test(
						existingUnitText.toLowerCase(),
					)
				) {
					if (unit !== undefined) {
