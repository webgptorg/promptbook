...
export const bookVersionCommandParser: PipelineHeadCommandParser<BookVersionCommand> =
	{
		 ...
		 */
		examples: [
			`BOOK VERSION ${BOOK_LANGUAGE_VERSION}`,
			`BOOK ${BOOK_LANGUAGE_VERSION}`,
		],
		...
		parse(input: CommandParserInput): BookVersionCommand {
			...
			if (
				args.length > 0 &&
				!(((args.length === 1 && args[0]) || "").toUpperCase() === "VERSION")
			) {
		...
		takeFromPipelineJson(
			pipelineJson: PipelineJson,
		): ReadonlyArray<BookVersionCommand> {
