...
describe("how KNOWLEDGE command in .book.md files works", () => {
	it("should parse KNOWLEDGE command", () => {
		expect(
			parseCommand("KNOWLEDGE https://www.pavolhejny.com/", "PIPELINE_HEAD"),
		).toEqual({
			type: "KNOWLEDGE",
	...
	it("should fail parsing KNOWLEDGE command", () => {
		expect(() => parseCommand("KNOWLEDGE", "PIPELINE_HEAD")).toThrowError(
			/Source is not defined/i,
		);
		expect(() => parseCommand("KNOWLEDGE brr", "PIPELINE_HEAD")).toThrowError(
			/Source not valid/i,
		);
		expect(() =>
			parseCommand("KNOWLEDGE http://www.pavolhejny.com/", "PIPELINE_HEAD"),
		).toThrowError(/Source is not secure/i);
		expect(() =>
			parseCommand("KNOWLEDGE ../hejny-cv.pdf", "PIPELINE_HEAD"),
		).toThrowError(/Source cannot be outside .* folder/i);
		expect(() =>
			parseCommand("KNOWLEDGE /hejny-cv.pdf", "PIPELINE_HEAD"),
		).toThrowError(/Source cannot be outside .* folder/i);
		expect(() =>
			parseCommand(
				"KNOWLEDGE /etc/system-folder/hejny-cv.pdf",
				"PIPELINE_HEAD",
			),
		).toThrowError(/Source cannot be outside .* folder/i);
		expect(() =>
			parseCommand("KNOWLEDGE C:/hejny-cv.pdf", "PIPELINE_HEAD"),
		).toThrowError(/Source cannot be outside .* folder/i);
		expect(() =>
			parseCommand("KNOWLEDGE C://hejny-cv.pdf", "PIPELINE_HEAD"),
		).toThrowError(/Source cannot be outside .* folder/i);
