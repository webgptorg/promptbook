...
describe('how FOREACH command in .book.md files works', () => {
    ...
    it('should parse FOREACH command in PIPELINE_TASK with multiple inputSubparameterNames', () => {
        expect(parseCommand('FOREACH Text Line `{customers}` -> `{firstName}`, `{lastName}`', 'PIPELINE_TASK')).toEqual(
            {
                type: 'FOREACH',
                formatName: 'TEXT',
                subformatName: 'LINE',
    ...
    it('should parse FOREACH command in PIPELINE_TASK with specified outputSubparameterName', () => {
        expect(
            parseCommand('FOREACH Csv Row `{customers}` -> `{firstName}`, `{lastName}`, `+{email}`', 'PIPELINE_TASK'),
        ).toEqual({
            type: 'FOREACH',
            formatName: 'CSV',
            subformatName: 'ROW',
    ...
    it('should parse FOREACH command in multiple formats', () => {
        ...
        expect(parseCommand('EACH   Text   Line {customers}     ->   {customer}   ', 'PIPELINE_TASK')).toEqual({
            type: 'FOREACH',
            formatName: 'TEXT',
            subformatName: 'LINE',
            parameterName: 'customers',
        ...
        */
        ...
        expect(parseCommand('FOREACH Csv Row {customers} -> {firstName} {lastName} +{email}', 'PIPELINE_TASK')).toEqual(
            {
                type: 'FOREACH',
                formatName: 'CSV',
                subformatName: 'ROW',
    ...
    it('should parse FOREACH command in shortcut form', () => {
        expect(parseCommand('EACH CSV CELL `{customers} -> `{customer}`', 'PIPELINE_TASK')).toEqual({
            type: 'FOREACH',
            formatName: 'CSV',
            subformatName: 'CELL',
            parameterName: 'customers',
    ...
    it('should fail parsing FOREACH command', () => {
        expect(() => parseCommand('FOREACH brr', 'PIPELINE_TASK')).toThrowError(/Unsupported format "BRR"/i);
        expect(() => parseCommand('FOREACH Text', 'PIPELINE_TASK')).toThrowError(/Invalid FOREACH command/i);
        expect(() => parseCommand('FOREACH Text Line', 'PIPELINE_TASK')).toThrowError(/Invalid FOREACH command/i);
        expect(() => parseCommand('FOREACH Text Line `{customer}`', 'PIPELINE_TASK')).toThrowError(
            /Invalid FOREACH command/i,
        );
        expect(() => parseCommand('FOREACH Text Line -> `{customer}`', 'PIPELINE_TASK')).toThrowError(
            /Invalid FOREACH command/i,
        );
        expect(() => parseCommand('FOREACH Csv Row `{customer}` ->', 'PIPELINE_TASK')).toThrowError(
            /FOREACH command must have at least one input subparameter/i,
        );

        expect(() =>
            parseCommand('FOREACH Csv Row `{customer}` -> {firstName}, {lastName}', 'PIPELINE_TASK'),
