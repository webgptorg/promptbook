...
export const foreachCommandParser: PipelineTaskCommandParser<ForeachCommand> = {
	...
	parse(input: CommandParserInput): ForeachCommand {
		...
		if (assignSign !== "->") {
			throw new ParseError(
				`FOREACH command must have '->' to assign the value to the parameter`,
			);
		}
		...
		if (outputSubparameterNames.length === 1) {
			...
			throw new ParseError(
				`FOREACH command can not have more than one output subparameter`,
			);
		}

		if (inputSubparameterNames.length === 0) {
			throw new ParseError(
				`FOREACH command must have at least one input subparameter`,
			);
		}
