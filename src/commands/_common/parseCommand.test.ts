...
describe("parsing the commands", () => {
	...
	it("should fail parsing multi-line command", () => {
		expect(() =>
			parseCommand("\nprompt section", "PIPELINE_HEAD"),
		).toThrowError(/Can not contain new line/i);
		expect(() =>
			parseCommand("prompt section\n", "PIPELINE_HEAD"),
		).toThrowError(/Can not contain new line/i);
	});

	it("should fail parsing unknown command", () => {
		expect(() => parseCommand("", "PIPELINE_HEAD")).toThrowError(
			/Malformed command/i,
		);
		expect(() => parseCommand("afasf ddd", "PIPELINE_HEAD")).toThrowError(
			/Malformed or unknown command/i,
		);
		expect(() => parseCommand("nothing to get", "PIPELINE_HEAD")).toThrowError(
			/Malformed or unknown command/i,
		);
		expect(() => parseCommand("prameter {name}", "PIPELINE_HEAD")).toThrowError(
			/Malformed or unknown command/i,
		);
	});
});
