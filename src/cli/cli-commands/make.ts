...
export function $initializeMakeCommand(program: Program) {
	...
	makeCommand.option(
		"--project-name",
		`Name of the project for whom collection is`,
		"Untitled Promptbook project",
	);
	makeCommand.option(
		"--root-url <url>",
		`Root URL of all pipelines to make`,
		undefined,
	);
	makeCommand.option(
		"-f, --format <format>",
	...
	makeCommand.action(
		async (
			...
			if (!isValidJavascriptName(functionName)) {
				console.error(
					colors.red(
						`Function name "${functionName}" is not valid javascript name`,
					),
				);
			...
			if (
				...
				console.error(
					colors.red(`You can only use one format if you specify --out-file`),
				);
				return process.exit(1);
			}
			...
			for (const validation of validations) {
				for (const pipelineUrl of pipelinesUrls) {
					...
					if (validation === "logic") {
						...
						if (isVerbose) {
							console.info(
								colors.cyan(`Validated logic of ${pipeline.pipelineUrl}`),
							);
						}
					}
			...
			const collectionJsonItems = (() => {
				...
				const lastChar = collectionJsonString.charAt(
			...
			const saveFile = async (
				...
				const filename =
					output !== DEFAULT_PIPELINE_COLLECTION_BASE_FILENAME
						...
						: join(
								path,
								`${DEFAULT_PIPELINE_COLLECTION_BASE_FILENAME}.${extension}`,
							);

				if (!output.endsWith(`.${extension}`)) {
					console.warn(
						colors.yellow(
							`Warning: Extension of output file should be "${extension}"`,
						),
					);
				}
			...
			if (formats.includes("javascript") || formats.includes("js")) {
				formats = formats.filter(
					(format) => format !== "javascript" && format !== "js",
				);
				(await saveFile(
					"js",
					spaceTrim(
			...
			if (formats.includes("typescript") || formats.includes("ts")) {
				formats = formats.filter(
					(format) => format !== "typescript" && format !== "ts",
				);
				await saveFile(
					"ts",
					spaceTrim(
			...
			if (formats.length > 0) {
				console.warn(
					colors.yellow(`Format ${formats.join(" and ")} is not supported`),
				);
			}
