...
export function $initializeRunCommand(program: Program) {
	...
	runCommand.argument(
		"[pipelineSource]",
		"Path to book file OR URL to book file, if not provided it will be asked",
	);
	runCommand.option(
		"-r, --reload",
	...
	runCommand.action(async (pipelineSource, options) => {
		...
		if (
			...
			console.error(
				colors.red(
					`""${pipelineSource}" is not a valid command or book. See 'ptbk --help'.`,
				),
			);
		...
		if (
			saveReport &&
			!saveReport.endsWith(".json") &&
			!saveReport.endsWith(".md")
		) {
					...
					$llmToolsMetadataRegister
						...
						.flatMap(({ title, envVariables }) =>
							envVariables === null
								...
								: [
										`# ${title}`,
										...envVariables.map((varname) => `# ${varname}=...`),
										"",
									],
						)
						.join("\n"),
		...
		if (!pipelineSource) {
			const response = await prompts({
				...
				validate: (value) =>
					value.length > 0 ? true : "Pipeline source is required",
			});
		...
		try {
			pipeline = await $getCompiledBook(
				tools,
				pipelineSource,
				prepareAndScrapeOptions,
			);
		...
		if (
			isInteractive === true &&
			isFormfactorUsed === true &&
			pipeline.formfactorName === "CHATBOT"
		) {
			return /* not await */ runInteractiveChatbot({
				pipeline,
				pipelineExecutor,
				isVerbose,
			});
		...
		if (saveReport && saveReport.endsWith(".json")) {
			...
			const executionReportString =
				executionReportJsonToString(executionReport);
		...
		for (const warning of warnings || []) {
			console.error(
				colors.red(colors.bold(warning.name) + ": " + warning.message),
			);
		}
		...
		if (json === undefined) {
			for (const key of Object.keys(outputParameters)) {
				const value =
					outputParameters[key] || colors.grey(colors.italic("(nothing)"));
				const separator =
					countLines(value) > 1 || countWords(value) > 100 ? ":\n" : ": ";
