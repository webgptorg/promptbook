...
export function renderPromptbookMermaid(
	...
	const parameterNameToTaskName = (parameterName: string_name) => {
		...
		const task = pipelineJson.tasks.find(
			(task) => task.resultingParameterName === parameterName,
		);
		...
		return (
			task.name || normalizeTo_camelCase("task-" + titleToName(task.title))
		);
	};
              ...
              subgraph "${pipelineJson.title}"
                  ...
                  ${block(
										pipelineJson.parameters
											...
											.map(
												({ name }) =>
													`${parameterNameToTaskName(name)}--"{${name}}"-->output`,
											)
											.join("\n"),
									)}
                  ...
                  ${block(
										pipelineJson.tasks
											.map((task) => {
												...
												const taskName = parameterNameToTaskName(
