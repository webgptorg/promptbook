...
export function validatePipeline_InnerFunction(pipeline: PipelineJson): void {
    ...
    */
	...
	for (const task of pipeline.tasks) {
		...
		if (
			RESERVED_PARAMETER_NAMES.includes(
				task.resultingParameterName as string_reserved_parameter_name,
			)
		) {
			throw new PipelineLogicError(
				spaceTrim(
                        ...
                        ${block(
													resovedParameters
														.filter(
															(name) =>
																!RESERVED_PARAMETER_NAMES.includes(
																	name as string_reserved_parameter_name,
																),
														)
