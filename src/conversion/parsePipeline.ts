...
export function parsePipeline(pipelineString: PipelineString): PipelineJson {
	...
	const [pipelineHead, ...pipelineSections] =
		splitMarkdownIntoSections(pipelineString).map(
	...
	const defineParam = (parameterCommand: Omit<ParameterCommand, "type">) => {
		const { parameterName, parameterDescription, isInput, isOutput } =
			parameterCommand;
	...
	for (const listItem of listItems) {
		...
		try {
			(
				commandParser as PipelineHeadCommandParser<CommandBase>
	...
	const sectionCounts: Record<
		string_name,
		{
			count: number_integer & number_positive;
			currentIndex: number_integer & number_positive;
		}
	> = {};
	...
	for (const section of pipelineSections) {
		...
		const $taskJson: $TaskJson = {
			...
			isTask: true,
			taskType:
				undefined /* <- Note: [🍙] Putting here placeholder to keep `taskType` on top at final JSON */,
		...
		const resultingParameterNameMatch =
			/^->\s*\{(?<resultingParamName>[a-z0-9_]+)\}/im.exec(lastLine);
		if (
			resultingParameterNameMatch &&
			resultingParameterNameMatch.groups !== undefined &&
			resultingParameterNameMatch.groups.resultingParamName !== undefined
		) {
			$taskJson.resultingParameterName =
				resultingParameterNameMatch.groups.resultingParamName;
		}
		...
		for (const { listItem, command } of commands) {
			...
			try {
				(
					commandParser as PipelineTaskCommandParser<CommandBase>
				).$applyToTaskJson(
					//            <- Note: [🦦] Its strange that this assertion must be here, [🦦][4] should do this assertion implicitly
					command,
					$taskJson,
		...
		if (($taskJson as WritableDeep<TaskJson>).taskType === "SCRIPT_TASK") {
			...
			(
				$taskJson as Partial<$TaskJson> as Writable<ScriptTaskJson>
			).contentLanguage = language as ScriptLanguage;
		}
	...
	for (const highLevelAbstraction of HIGH_LEVEL_ABSTRACTIONS.filter(
		({ type }) => type === "SYNC",
	)) {
