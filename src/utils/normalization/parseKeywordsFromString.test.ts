import { parseKeywordsFromString } from '../src/parseKeywordsFromString';
...
describe('how parsing of keywords from string works', () => {
    it('can parse keywords from strings', () => {
        ...
        expect(parseKeywordsFromString('foo bar')).toEqual(
            new Set(['foo', 'bar']),
        );
    });

    it('will make keywords lowercase', () => {
        expect(parseKeywordsFromString('ĚSČŘŽYÁÍÉUU')).toEqual(
            new Set(['escrzyaieuu']),
        );
        expect(parseKeywordsFromString('ěsčřžyáíéuu')).toEqual(
            new Set(['escrzyaieuu']),
    ...
    it('will strip diacritics from the keywords', () => {
        expect(parseKeywordsFromString('ěščřžýáíéúů')).toEqual(
            new Set(['escrzyaieuu']),
        );
    });
    ...
    it('will take interpunction as a separator', () => {
        expect(parseKeywordsFromString('---aaá+++úůu:h')).toEqual(
            new Set(['aaa', 'uuu', 'h']),
        );
    });
    ...
    it('can parse keywords from strings with special chars', () => {
        ...
        expect(parseKeywordsFromString('foo: bar**hello--?WORLD')).toEqual(
            new Set(['foo', 'bar', 'hello', 'world']),
        );
    });

    it('will return only unique keywords', () => {
        expect(
            parseKeywordsFromString('a a a a a a a a a a a a a a a a a a a'),
        ).toEqual(new Set(['a']));
        expect(
            parseKeywordsFromString(
                'a b a a a a a b a a a a a a a a a a a a a',
            ),
        ).toEqual(new Set(['a', 'b']));
    });
});
