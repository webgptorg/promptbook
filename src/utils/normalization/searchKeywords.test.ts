...
describe("how searching through keywords works", () => {
	...
	it("can search multiple keyword", () => {
		expect(searchKeywords(new Set(["a", "b"]), new Set(["a", "b"]))).toEqual(
			true,
		);
		expect(searchKeywords(new Set(["a", "b"]), new Set(["b", "a"]))).toEqual(
			true,
		);
		expect(searchKeywords(new Set(["a", "b"]), new Set(["a", "c"]))).toEqual(
			false,
		);
	});

	it("can search incomplete keyword", () => {
		expect(searchKeywords(new Set(["aaa", "bbbb"]), new Set(["a"]))).toEqual(
			true,
		);
		expect(searchKeywords(new Set(["aaa", "bbbb"]), new Set(["ab"]))).toEqual(
			false,
		);
	});
