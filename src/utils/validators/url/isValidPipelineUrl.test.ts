...
describe(`validation of urls`, () => {
    ...
    it(`is valid pipeline url BUT not secure`, () => {
        expect(isValidPipelineUrl(`http://promptbook.studio/library/promptbook.ptbk.md`)).toBe(true);
        expect(isValidPipelineUrl(`https://192.168.5.3/library/promptbook.ptbk.md`)).toBe(true);
    });
    ...
    it(`is valid url BUT not valid pipeline URL`, () => {
        expect(isValidPipelineUrl(`https://promptbook.studio/`)).toBe(true);
        expect(isValidPipelineUrl(`https://collboard.com/`)).toBe(true);
        expect(isValidPipelineUrl(`http://localhost:9977/fooo/add`)).toBe(true);
    });
});
