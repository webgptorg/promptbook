...
describe(`validation of urls`, () => {
	...
	it(`dataurl is valid url`, () => {
		expect(
			isValidUrl(`data:text/plain;base64,PGgxPkhlbGxvLCBXb3JsZCE8L2gxPg==`),
		).toBe(true);
		expect(
			isValidUrl(
				`data:text/plain;charset=utf-8;base64,PGgxPkhlbGxvLCBXb3JsZCE8L2gxPg==`,
			),
		).toBe(true);
		expect(
			isValidUrl(`data:text/html,%3Ch1%3EHello%2C%20World%21%3C%2Fh1%3E`),
		).toBe(true);
