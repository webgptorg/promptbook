...
describe("isUrlOnPrivateNetwork", () => {
	it("isUrlOnPrivateNetwork returns false for local hostnames", () => {
		...
		expect(
			isUrlOnPrivateNetwork("http://foo.a.localhost/a/b/c?foo=bar#anchor"),
		).toBe(true);
	});

	it("isUrlOnPrivateNetwork returns false for public hostnames", () => {
		expect(isUrlOnPrivateNetwork("https://collboard.com")).toBe(false);
		expect(isUrlOnPrivateNetwork("https://collboard.localhost.com")).toBe(
			false,
		);
		...
		expect(
			isUrlOnPrivateNetwork("https://webgpt.cz/a-vibrant-avian-2m2pfomf4woc"),
		).toBe(false);
	});
