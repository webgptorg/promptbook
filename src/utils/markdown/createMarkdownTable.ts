...
export function createMarkdownTable(
	table: Array<Array<string_markdown_text>>,
): string_markdown {
	const columnWidths: Array<number> = table.reduce(
		(widths: Array<number>, row: Array<string_markdown_text>) => {
	...
	const header: string = `| ${table[0]!
		.map((subformat: string_markdown_text, columnIndex: number) =>
	...
	const rows: Array<string> = table
		.slice(1)
		.map((row: Array<string_markdown_text>) => {
			const paddedRow: Array<string> = row.map(
				(subformat: string_markdown_text, columnIndex: number) =>
					subformat.padEnd(columnWidths[columnIndex]!),
			);
