 ...
 */
export function renamePipelineParameter(
	options: RenameParameterOptions,
): PipelineJson {
	const { pipeline, oldParameterName, newParameterName } = options;

	if (
		pipeline.parameters.some((parameter) => parameter.name === newParameterName)
	) {
	...
	for (const task of renamedPipeline.tasks) {
		...
		task.dependentParameterNames = task.dependentParameterNames.map(
			(dependentParameterName) =>
				dependentParameterName === oldParameterName
					? newParameterName
					: dependentParameterName,
		);
