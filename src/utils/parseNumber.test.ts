...
describe("how parseNumber works", () => {
	...
	it("should parse Infinity", () => {
		...
		expect(parseNumber("♾")).toBe(Number.POSITIVE_INFINITY);
		expect(parseNumber("-♾")).toBe(Number.NEGATIVE_INFINITY);
		expect(parseNumber("+INFINITY")).toBe(Number.POSITIVE_INFINITY);
		expect(parseNumber("-INFINITY")).toBe(Number.NEGATIVE_INFINITY);
		expect(parseNumber("INF")).toBe(Number.POSITIVE_INFINITY);
		expect(parseNumber("-INF")).toBe(Number.NEGATIVE_INFINITY);
		expect(parseNumber(Number.POSITIVE_INFINITY)).toBe(
			Number.POSITIVE_INFINITY,
		);
		expect(parseNumber(Number.NEGATIVE_INFINITY)).toBe(
	...
	it("should throw error on invalid entries", () => {
		...
		expect(() => parseNumber("86abc123")).toThrowError(
			/Unable to parse number/i,
		);
		expect(() => parseNumber("abc123")).toThrowError(/Unable to parse number/i);
		expect(() => parseNumber("abc123xyz")).toThrowError(
			/Unable to parse number/i,
		);
		...
		expect(() => parseNumber("1.2.3.4")).toThrowError(
			/Unable to parse number/i,
		);
	});
});
