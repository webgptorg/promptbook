...
export function $execCommand(options: ExecCommandOptions): Promise<string> {
	if (!$isRunningInNode()) {
		throw new EnvironmentMismatchError(
			"Function `$execCommand` can run only in Node environment.js",
		);
	}
		...
		if (timeout !== Number.POSITIVE_INFINITY) {
			...
			forTime(timeout).then(() => {
				if (crashOnError) {
					reject(
						new Error(
							`Command "${humanReadableCommand}" exceeded time limit of ${timeout}ms`,
						),
					);
				} else {
					console.warn(
		...
		try {
			...
			commandProcess.on("error", (error) => {
				if (crashOnError) {
					reject(
						new Error(
							`Command "${humanReadableCommand}" failed: \n${error.message}`,
						),
					);
				} else {
					if (isVerbose) {
