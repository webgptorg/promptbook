...
describe("templateParameters", () => {
	...
	it("should replace parameter at the end", () => {
		expect(templateParameters("Hello {name}", { name: "World" })).toBe(
			"Hello World",
		);
	});
	...
	it("should replace parameter in the middle", () => {
		expect(
			templateParameters("Hello {name}, how are you?", { name: "World" }),
		).toBe("Hello World, how are you?");
	});

	it("should replace parameter at the beginning", () => {
		expect(templateParameters("{name}, how are you?", { name: "World" })).toBe(
			"World, how are you?",
		);
	});
	...
	it("should replace multiple parameters", () => {
		expect(
			templateParameters("{greeting} {name}, how are you?", {
				greeting: "Hello",
				name: "World",
			}),
		).toBe("Hello World, how are you?");
	...
	it("should not be confused by JSON", () => {
		expect(
			templateParameters(
				'{greeting} {name}, this is how JSON look like {"key": 1}.',
				{
					greeting: "Hi",
					name: "Pavol",
				},
		...
		expect(
			...
			'Hi Pavol, this is how JSON look like {"greeting": "Hi", "name": "Pavol"}.',
		);
		expect(
			templateParameters(
			...
			'Hi Pavol, this is how JSON look like {"params": {"greeting": "Hi", "name": "Pavol"}}.',
		);
		expect(
			templateParameters(
			...
			'Hi Pavol, this is how invalid JSON look like {"params": {"greeting": "Hi", "name": "Pavol"}.',
		);
	});
	...
	it("should not be confused by some NON-JSON structure containing chars {}", () => {
		...
		).toBe("{greeting world}, how are you?");
		expect(
			templateParameters("<greeting {name}>", {
				greeting: "Hello",
				name: "world",
			}),
		).toBe("<greeting world>");
