...
describe("extractParameterNames", () => {
	...
	it("should parse one parameter", () => {
		...
		expect([...extractParameterNames("Hello {name}, how are you?")]).toEqual([
			"name",
		]);
		expect([...extractParameterNames("{name}, how are you?")]).toEqual([
			"name",
		]);
	});
	...
	it("should parse multiple parameters", () => {
		expect([
			...extractParameterNames("{greeting} {name}, how are you?"),
	...
	it("should not be confused by JSON", () => {
		...
		expect([
			...extractParameterNames(
				"{greeting} {name}, this is how JSON look like {}.",
			),
		]).toEqual(["greeting", "name"]);
		expect([
			...extractParameterNames(
	...
	it("should parse parameter included multiple times", () => {
		expect([
			...extractParameterNames(
				"{greeting} {name}, how are you? {greeting} {name}",
			),
		]).toEqual(["greeting", "name"]);
	});
	...
	it("should not be confused by some NON-JSON structure containing chars {}", () => {
		...
		expect([...extractParameterNames("<{greeting {name}}>")]).toEqual(["name"]);
		expect([...extractParameterNames("<{{{greeting {name}}}}>")]).toEqual([
			"name",
		]);
		expect([...extractParameterNames("{greeting} }{}{}{")]).toEqual([
			"greeting",
		]);
	});
