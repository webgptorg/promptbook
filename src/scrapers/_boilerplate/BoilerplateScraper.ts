	...
	public async $convert(
		...
		if (!$isRunningInNode()) {
			throw new KnowledgeScrapeError(
				"BoilerplateScraper is only supported in Node environment",
			);
		}
		...
		if (this.tools.fs === undefined) {
			throw new EnvironmentMismatchError(
				"Can not scrape boilerplates without filesystem tools",
			);
		...
		if (source.filename === null) {
			...
			throw new KnowledgeScrapeError(
				"When parsing .@@@ file, it must be real file in the file system",
			);
		}
