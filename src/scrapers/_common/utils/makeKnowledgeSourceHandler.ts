...
export async function makeKnowledgeSourceHandler(
	...
	if (isValidUrl(knowledgeSourceContent)) {
		...
		await tools.fs!.mkdir(dirname(join(rootDirname, filepath)), {
			recursive: true,
		});
		...
		if (
			fileContent.length >
			DEFAULT_MAX_FILE_SIZE /* <- TODO: Allow to pass different value to remote server */
		) {
		...
		return makeKnowledgeSourceHandler(
			{ name, knowledgeSourceContent: filepath },
			tools,
			{
				...options,
				rootDirname,
		...
		const filename = join(rootDirname, knowledgeSourceContent)
			.split("\\")
			.join("/");
