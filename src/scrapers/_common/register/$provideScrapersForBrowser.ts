...
export async function $provideScrapersForBrowser(
	...
	if (!$isRunningInBrowser() || $isRunningInWebWorker()) {
		throw new EnvironmentMismatchError(
			"Function `$provideScrapersForBrowser` works only in browser environment",
		);
	}
	...
	if (
		isAutoInstalled ===
		true /* <- Note: [0] Ignoring undefined, just checking EXPLICIT requirement for install */
	) {
		throw new EnvironmentMismatchError(
			"Auto-installing is not supported in browser environment",
		);
	}
