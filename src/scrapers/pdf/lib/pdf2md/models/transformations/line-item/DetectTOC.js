		...
		parseResult.pages.slice(0, maxPagesToEvaluate).forEach((page) => {
			...
			tocLines.forEach((line) => {
				...
				if (endsWithDigit) {
					...
					pageTocLinks.push(
						new TocLink({
							pageNumber: Number.parseInt(digits.join("")),
...
class LinkLeveler {
	...
	calculateUniqueX(tocLinks) {
		var uniqueX = tocLinks.reduce((uniquesArray, link) => {
	...
	calculateUniqueFonts(tocLinks) {
		var uniqueFont = tocLinks.reduce((uniquesArray, link) => {
