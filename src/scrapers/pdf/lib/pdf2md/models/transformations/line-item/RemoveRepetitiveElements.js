...
module.exports = class RemoveRepetitiveElements extends (
	...
	transform(parseResult /*: ParseResult */) /*: ParseResult */ {
		...
		parseResult.pages.forEach((page) => {
			...
			const minLineHash = hashCodeIgnoringSpacesAndNumbers(
				minMaxItems.minElements.reduce(
					(combinedString, item) => combinedString + item.text().toUpperCase(),
					"",
				),
			);
			const maxLineHash = hashCodeIgnoringSpacesAndNumbers(
				minMaxItems.maxElements.reduce(
					(combinedString, item) => combinedString + item.text().toUpperCase(),
					"",
				),
			);
