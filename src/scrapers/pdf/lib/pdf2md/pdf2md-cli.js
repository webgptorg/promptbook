...
if (!argv["inputFolderPath"]) {
	...
	typeof argv["recursive"] !== "boolean" &&
	argv["recursive"] !== undefined
) {
	console.log(
		"Add tag --recursive for recursive folder conversion, otherwise omit",
	);
} else {
...
function run(folderPath, outputPath, recursive = true) {
	...
	var [allFilePaths] = getAllFileAndFolderPaths(
		filenames,
		folderPaths,
		recursive,
	);
	var allOutputPaths = allFilePaths.map((x) => {
		...
		const fileNameWithoutExtension = fileNameWithExtension.slice(
			0,
			fileNameWithExtension.indexOf(".pdf"),
		);
