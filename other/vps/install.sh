...
install_agents_server_repository_dependencies() {
    if [[ -z "$PROMPTBOOK_REPOSITORY_DIR" || ! -d "$PROMPTBOOK_REPOSITORY_DIR" ]]; then
        fail "Cannot install repository dependencies because PROMPTBOOK_REPOSITORY_DIR is not available."
    fi

    install_agents_server_browser_dependencies
}

install_agents_server_dependencies() {
    install_agents_server_host_dependencies
    install_agents_server_repository_dependencies
}
...
install_agents_server_dependency_requirements() {
    ...
    if [[ -f "$release_install_script_path" ]] && grep -q '^apply_dependency_configuration()' "$release_install_script_path"; then
        ...
        PTBK_NON_INTERACTIVE=1 \
            PTBK_INSTALL_DIR="$INSTALL_DIR" \
            PTBK_DATA_DIR="$PTBK_DATA_DIR" \
...
build_nginx_agent_project_vscode_location_block() {
    cat <<EOF
    location ~ "^/__agent-project-vscode/([^/]+)(/.*)?$" {
        set \$promptbook_code_server_session_id \$1;
        ...
        rewrite ^/__agent-project-vscode/[^/]+/?(.*)$ /\$1 break;
        proxy_pass http://127.0.0.1:\$promptbook_code_server_port;
        proxy_http_version 1.1;
        proxy_buffering off;
        proxy_redirect off;
        proxy_hide_header Server;
        ...
        proxy_set_header X-Forwarded-Host \$host;
        proxy_set_header X-Forwarded-Proto \$scheme;
        proxy_set_header X-Forwarded-Prefix /__agent-project-vscode/\$promptbook_code_server_session_id;
...
apply_dependency_configuration() {
    initialize_sudo
    resolve_run_user
