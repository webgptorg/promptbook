...
-   The database settings should be stored in the `.env` file, and its one of the settings that must be configured during the installation process and cannot be setup during the UI wizard "Create new server" or in super `admin` UI, but it is allowed just to press enter to use the default PostgreSQL configuration
