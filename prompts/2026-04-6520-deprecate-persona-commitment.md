...
-   Replace persona commitment parsing in both the book editor visuals and any parsing/rendering pipeline with goal commitment parsing.
-   In the book editor (visualization), ensure the persona block is not shown as the source of truth; display the latest goal-derived profile content instead.
...
-   Link UI changes to confirm in manual QA that the agent profile preview matches the parsing output.
