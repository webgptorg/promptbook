[x] ~$0.2671 30 minutes by OpenAI Codex `gpt-5.5`

[🧹🥇] Refactor [`startAgentsServer.ts` file](src/cli/cli-commands/agents-server/startAgentsServer.ts)

- The file mixes multiple concerns and dense logic, making it harder to follow.
- The file defines too many responsibilities (40 in single file)
    - Keep in mind the Single Responsibility Principle (SRP)
    - Consider breaking it down into smaller, focused modules or components.
- The file contains too many functions (37/20)
