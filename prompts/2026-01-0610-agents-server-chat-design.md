...
-   When the message contains little text, the message has very low width and [looks very bad](prompts/screenshots/
...
-   Fix this by fading out the buttons when scrolling the chat and being above the first obstructed message.
-   You are working with the [Agents Server](apps/agents-server) with the agent chat _(for example, [here](https://my-agent-server.com/agents/FVLv8APAf2S1WV/chat))_
...
-   The pop-up should have a better design, with rounded corners and a shadow.
-   Fix there a image
-   Change the view source button to view profile, which will lead to the profile page of the agent.
-   You are working with the [Agents Server](apps/agents-server) with the agent chat _(for example, [here](https://my-agent-server.com/agents/FVLv8APAf2S1WV/chat))_
