...
[x] (2 attempts) by OpenAI Codex `gpt-5.6-terra` thinking `max` (ChatGPT account) - Implementation ~$0.6221 17 minutes; Testing 6 minutes; Fixing ~.02 an hour; Testing 21 minutes
...
-   The following is already implemented:
    ...
    -   After the prompt start to being processed, the `[^]` -> `[x]` should be added there as if it had a `[ ]` checkbox initially.
    -   The prompt with no `[ ]` checkbox has priority `0` _(same as if there is no `[ ] !`)_
-   But it should also handle `[.]` which should be treated as `[-]` - the prompt will not run
-   Also when there is `<!--ptbk-coder-ignore-->`, `ptbk-coder-ignore` ignore entire markdown file
-   Keep in mind the DRY _(don't repeat yourself)_ principle.
-   Do a proper analysis of the current functionality of `ptbk coder` and related functionality before you start implementing.
-   You are working with [`ptbk coder`](src/cli/cli-commands/coder/run.ts) if there are any changes that affect the landing page.
-   Update the [`ptbk coder` landing website](apps/coder-landing)
