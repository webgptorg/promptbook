...
-   _(@@@ wait for fable and external chats)_
...
-   In the previous versions of the agent server, there was a concept of use timeout, take the useful things and pages from this concept and reimplement this for the agent to set up the cron and time out by itself only from a goal.
    -   The previous implementation of timers is in `/agents/v4jw3bn39jM89h/timeouts`
    -   Rename it to "Goal" but `/agents/v4jw3bn39jM89h/goals` and this should be the dashboard for the agent to set up its own cron jobs and timeouts.
...
-   When the agent is woken up by the cron, it should be able to do nothing or to do something, and it should be able to modify its cron jobs from the information it has in its goals and knowledge.
-   Every wake up of the agent is doing something, it should create a chat thread for this wake up
-   This chat is not connected to any user but agent itself
-   When admin toggles external chats, theese cron chats should be visible to the admin
