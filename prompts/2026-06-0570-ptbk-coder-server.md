...
[x] ~$0.6483 an hour by OpenAI Codex `gpt-5.5`
...
-   In the Web UI the columns should be following:
    1. Backlog, for both [-] and prompts containing 3x "@"
        - Add two UI tags in UI to indicate if the prompt is in backlog because of [-] or because it contains 3x "@"
    ...
    4. In progress, for prompts which are currently being processed by the agent (for now there will be only one or no prompt in this column)
        - The task in progress should have one of two tags, either "Implementing" or "Verifying", depending on what the agent is doing
    5. Done, for prompts which are done by agent
    6. Errors, for prompts which are done with error by any agent session server
    7. Finished, for prompts which manually verified by human and marked as finished
-   In the web UI do not allow to drag and drop the prompt cards, this will be added in future, for now the human operator can only see the status of the prompts and edit their markdown contents
-   By editing the prompt in the web UI, it will edit the prompt file on the backend correspondingly and the changes will be immediately commited to the git
-   Also in the web UI show the real progress of the agent from terminal
-   Keep in mind the DRY _(don't repeat yourself)_ principle
-   Do a proper analysis of the current functionality of `ptbk coder server` and related functionality before you start implementing.
-   You are working with [`ptbk coder`](src/cli/cli-commands/coder/run.ts)
-   Add the changes into the [changelog](changelog/_current-preversion.md)
...
-   (@@@ Wait for pre)
-   In the Web UI the columns are following:
    1. Backlog, for both [-] and prompts containing 3x "@"
        - Add two UI tags in UI to indicate if the prompt is in backlog because of [-] or because it contains 3x "@"
    2. Low priority, for prompts which are ready but have lower priority then `--priority` specified
    3. To do, for prompts which are in the queue and ready to be processed
    ...
    6. Errors, for prompts which are done with error by any agent session server
    7. Finished, for prompts which manually verified by human and marked as finished
-   In the UI allow the human operator to do the following actions by drag and drop of the prompt cards:
    -   @@@
-   Keep in mind the DRY _(don't repeat yourself)_ principle
-   Do a proper analysis of the current functionality of `ptbk coder server` and related functionality before you start implementing.
-   You are working with [`ptbk coder`](src/cli/cli-commands/coder/run.ts)
