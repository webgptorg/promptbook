...
[x] ~$1.38 by OpenAI Codex `gpt-5.2-codex`

[✨♿️] Agents graph on home page should be redesigned using React flow library
...
-   Show Both the relationship between the agents (`FROM`, `IMPORT`, `TEAM`) and also how are agents in the folders and order.
-   Allow to edit the agents inside graph layout by dragging nodes around and repositioning them.
-   Cleanup the unused code related to the old graph implementation.
-   You are working with the `Agents Server` application `/apps/agents-server`
-   Keep in mind the DRY _(don't repeat yourself)_ principle.
-   Add the changes into the `/changelog/_current-preversion.md`
...
[✨♿️] Agents graph on home page should be redesigned to be more visually appealing

-   @@@
-   It should look more like a social graph than a UML diagram.

![alt text](prompts/screenshots/2025-12-1240-agents-server-better-agents-graph-2.png)
![alt text](prompts/screenshots/2025-12-1240-agents-server-better-agents-graph-3.png)
