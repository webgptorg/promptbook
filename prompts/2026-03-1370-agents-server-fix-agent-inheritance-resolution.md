...
-   Currently, inheritance referencing _looks_ like it works (resolved book preview looks correct) but not all properties that should be inherited are actually inherited in runtime/UI.
...
-   The resolved agent source shown when inspecting the resolved URL should be treated as the canonical expected result and used as acceptance baseline.
    -   URL to example resolved agent source: https://core-test.ptbk.io/agents/DejX49wooCtris/api/book
...
-   The inheritance (`FROM` commitment) and import (`IMPORT` commitment) resolving must be reused by: - chat runtime - agent profile page - any server-side indexing/search - API endpoints returning agent metadata
