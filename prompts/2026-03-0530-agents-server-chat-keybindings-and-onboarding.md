...
-   When a user uses the chat for the first time and presses `Enter` (with no stored preference yet), show a non-blocking prompt asking which behavior they want.
    ...
    -   The user decision should take effect for subsequent Enter/Ctrl+Enter presses; the first Enter press that triggered the prompt should follow the newly selected mode if feasible, otherwise keep the text unchanged and require a second Enter to send (@@@ decide preferred UX).
    -   Provide a clear “Not now” / dismiss option; if dismissed, do not re-prompt within the same session (@@@ define persistence window).
...
-   The reusable Chat component must not own persistence; instead it must receive behavior via props.
    -   Introduce a prop (e.g. `sendOnEnter: boolean` or `enterBehavior: 'send' | 'newline'`), plus support for the secondary binding (`Ctrl+Enter` inverse).
...
-   Technical considerations:
    -   Do not break IME composition or mobile keyboards (e.g. don’t treat composition Enter as send) (@@@ verify current implementation).
    -   Do not interfere with multiline paste and existing input handling.
    -   Add lightweight tests for the key handling logic if there is an existing test setup; otherwise add at least unit tests for the keybinding utility (@@@ locate preferred test framework).
-   You are working with the [Agents Server](apps/agents-server)
-   You are working with the chat component (likely [Chat](src/book-components/Chat/Chat/Chat.tsx) or equivalent @@@ confirm actual path)
-   You are working with routing/menu/settings UI in (apps/agents-server) @@@
