...
[✨♌️] Refactor [`sync-vercel-domains.ts` file](apps/agents-server/scripts/sync-vercel-domains.ts)

- The file mixes multiple concerns, making it harder to follow.
- The file contains excessive lines of code (2642 lines)
    - Keep in mind the Single Responsibility Principle (SRP)
    - Consider breaking it down into smaller, focused modules or components.
- The file defines too many responsibilities (83 in single file)
    - Keep in mind the Single Responsibility Principle (SRP)
    - Consider breaking it down into smaller, focused modules or components.
