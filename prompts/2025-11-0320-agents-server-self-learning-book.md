...
-   In table `AgentHistory` in Supabase, we store all changes to the agent
-   But we do not distinct if the change was made by self-learning from conversations or manual
-   Add a column `changeType` with enum values `SELF_LEARNING` and `MANUAL_CHANGE`
-   Add this logic to `AgentCollectionInSupabase`
-   Create a database migration for this
-   You are working with the `Agents Server` application `/apps/agents-server`
-   Keep in mind the DRY _(don't repeat yourself)_ principle.
