[x] ~$0.00 18 minutes by OpenAI Codex `gpt-5.1-codex-mini`

[✨☁️] Refactor [`Header.tsx` file](apps/agents-server/src/components/Header/Header.tsx)

- The file mixes multiple concerns, making it harder to follow.
- The file contains excessive lines of code (3344 lines)
    - Keep in mind the Single Responsibility Principle (SRP)
    - Consider breaking it down into smaller, focused modules or components.
- The file defines too many responsibilities (48 in single file)
    - Keep in mind the Single Responsibility Principle (SRP)
    - Consider breaking it down into smaller, focused modules or components.
