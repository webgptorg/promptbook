...
-   The `run_browser` fails when Agent tries to spawn the browser
-  It is failing both on local environment and on Vercel server, so it is not related to the remote browser tunnel
-  It should work on both environments, local and Vercel server, without any issues
-  Enhance the error message to be more descriptive and helpful for debugging, for example, it should include information about the environment and the error that occurred when trying to run the browser.
-   Do a proper analysis of the current functionality before you start implementing.
-   You are working with the [Agents Server](apps/agents-server)
