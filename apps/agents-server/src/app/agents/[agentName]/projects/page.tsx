...
export default async function AgentProjectsPage({ params }: { params: Promise<{ agentName: string }> }) {
    ...
    const [projects, isAdmin, agentSnapshot] = await Promise.all([
    ...
    return (
        <div className="container mx-auto space-y-6 px-4 py-8">
            <div className="mt-20 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                <div>
                    ...
                    <p className="mt-1 max-w-3xl text-sm text-gray-500">
                        Every project is one folder the agent fully controls — it can create and edit files there, run
                        scripts, keep a git repository, and link the files into the chat.
                    </p>
                </div>
            ...
            <AgentProjectsBoard agentPermanentId={canonicalAgentId} projects={projects} isAdminView={isAdmin} />
        </div>
    );
}
