...
describe('CloudflareDnsWizard', () => {
    ...
    it('imports the whole DNS setup of one server from all its record groups at once', () => {
        render(
            <CloudflareDnsWizard
                domain="agents.example.com"
                recordGroups={[
        ...
        expect(screen.getByRole('button', { name: 'Import 4 records' })).not.toBeNull();

        fireEvent.click(screen.getByRole('button', { name: 'Import zone file' }));

        const zoneFile = screen.getByText(/Import this file at your DNS provider/).textContent || '';

        expect(zoneFile).toContain('agents.example.com.\t1\tIN\tA\t203.0.113.42');
        expect(zoneFile).toContain('*.agents.example.com.\t1\tIN\tCNAME\tagents.example.com.');
        expect(zoneFile).toContain('agents.example.com.\t1\tIN\tMX\t10 mail.agents.example.com.');
        expect(zoneFile).not.toContain('live.example.com.');
    });
