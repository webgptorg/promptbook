...
export function AgentProfileLoadingSkeleton({ isHeadless = false }: AgentProfileLoadingSkeletonProps) {
    return (
        ...
        >
            <div className="absolute inset-0 bg-gradient-to-br from-sky-100 via-white to-indigo-100 dark:from-slate-950 dark:via-slate-950 dark:to-sky-950/60" />
            {!isHeadless && (
            ...
            <div className="relative z-10 grid w-full max-w-5xl grid-cols-1 items-start gap-y-6 md:grid-cols-[auto_1fr] md:gap-x-12 md:gap-y-4">
                ...
                <div className="w-full md:col-start-2">
                    <div className="relative h-[calc(100dvh-300px)] min-h-[350px] rounded-[32px] border border-white/40 bg-white/70 p-4 shadow-2xl backdrop-blur-2xl dark:border-slate-700/60 dark:bg-slate-950/78 md:h-[500px] md:min-h-[420px]">
                        <ChatThreadLoadingSkeleton
                            withComposer
                            className="rounded-[24px] border border-white/40 bg-white/75 dark:border-slate-700/60 dark:bg-slate-950/72"
                        />
                    </div>
                </div>
                {!isHeadless && (
