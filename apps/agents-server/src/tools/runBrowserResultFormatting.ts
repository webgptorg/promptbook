...
export const runBrowserResultFormatting = {
    ...
    formatSuccessResult(options: {
        ...
        const finalSnapshotMarkdown = snapshotPath
            ? spaceTrim(`
                  ## Final snapshot
        ...
        return spaceTrim(
            (block) => `
                ...
                **Executed actions:** ${payload.executedActions.length}
                ...
                ${block(visualReplayMarkdown)}
                ${block(finalSnapshotMarkdown)}
     ...
     */
    formatErrorResult(options: { readonly payload: RunBrowserResultPayload }): string {
        const { payload } = options;
        const toolError = payload.error;
        const suggestedNextSteps = toolError?.suggestedNextSteps || [];
        const suggestedNextStepsMarkdown =
            suggestedNextSteps.length === 0
                ...
                : spaceTrim(
                      (block) => `
                          ...
                          ${block(suggestedNextSteps.map((step) => `- ${step}`).join('\n'))}
                      `,
                  );
        ...
        return spaceTrim(
            (block) => `
                ...
                **Error:** ${toolError?.message || 'Unknown browser tool error'}
                ...
                ${block(suggestedNextStepsMarkdown)}
