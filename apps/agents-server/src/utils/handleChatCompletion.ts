...
import { OpenAiAgentVectorStoreCacheManager } from './cache/OpenAiAgentVectorStoreCacheManager';
...
export async function handleChatCompletion(
    ...
    try {
        ...
        // Use OpenAiAgentVectorStoreCacheManager for vector store caching
        const vectorStoreCacheManager = new OpenAiAgentVectorStoreCacheManager({ isVerbose: true });
        const baseOpenAiTools = await $provideOpenAiAgentExecutionToolsForServer();

        // Get or create vector store with enhanced caching
        ...
        const openAiAgentExecutionTools = vectorStoreResult.tools;

        if (vectorStoreResult.fromCache) {
            console.log(
        ...
        const agent = new Agent({
            agentSource,
            executionTools: {
                llm: openAiAgentExecutionTools, // Note: Use the same OpenAI Agent tools as the chat route
            },
