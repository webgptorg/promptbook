 ...
 */
const PDF_RENDERER_ALLOWED_REQUEST_PROTOCOLS = new Set(['about:', 'data:']);

/**
 * Browser viewport used while rendering standalone HTML exports to PDF.
 */
const PDF_RENDERER_VIEWPORT = {
    width: 1280,
    height: 1600,
} as const;
...
export async function renderHtmlToPdfOnServer(html: string): Promise<Buffer> {
    ...
    try {
        ...
        try {
            ...
            await page.setViewportSize(PDF_RENDERER_VIEWPORT);
...
/**
 * Returns whether a PDF-renderer request can proceed without connecting to a host.
 *
...
function isAllowedPdfRendererRequestUrl(requestUrl: string): boolean {
    try {
        return PDF_RENDERER_ALLOWED_REQUEST_PROTOCOLS.has(new URL(requestUrl).protocol);
    } catch {
        return false;
    }
}
