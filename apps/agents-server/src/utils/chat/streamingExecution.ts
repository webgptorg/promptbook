...
/**
 * Minimal row returned when checking recently finished executions.
 *
 * @private internal utility of Agents Server chat streaming
 */
type FinishedStreamingExecutionRow = {
    readonly id: string;
    readonly updatedAt: Date;
    readonly status: StreamingExecutionStatus;
};
...
/**
 * Gets the newest finished execution for a chat updated after provided timestamp.
 *
 * @public exported from Agents Server utils
 */
export async function getLatestFinishedStreamingExecutionForChat(params: {
    userChatId: string;
    updatedAfter: Date;
}): Promise<FinishedStreamingExecutionRow | null> {
    const { userChatId, updatedAfter } = params;
    const sql = await $provideClientSql();
    const tableIdentifier = await getStreamingExecutionTableIdentifier();

    const [execution] = await sql.raw<FinishedStreamingExecutionRow[]>(
        `
