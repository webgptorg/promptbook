...
/**
 * Pattern of characters kept as-is inside one filesystem directory segment.
 */
...
/**
 * Resolves the root filesystem directory holding all per-agent projects.
 *
...
/**
 * Resolves the filesystem directory holding projects for one agent.
 *
...
function sanitizeAgentProjectDirectorySegment(value: string): string {
    return value.replace(AGENT_PROJECT_DIRECTORY_UNSAFE_CHARACTER_PATTERN, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
}
