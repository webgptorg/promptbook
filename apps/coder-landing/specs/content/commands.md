...
| `LIVE_RUN_COMMAND`       | `ptbk coder run --harness claude-code --model fable --thinking-level xhigh --agent agents/coding/developer.book --context AGENTS.md --test npm run test-for-ptbk-coder --wait-between-prompts 4h --limit 1` | [Live terminal](../components/live-terminal.md) |
| `DRY_RUN_COMMAND`        | `ptbk coder run --dry-run`                                                                                                                                                   | [Advanced features](../sections/advanced-features.md)                                                 |
| `SERVER_COMMAND`         | `ptbk coder server --harness claude-code --model fable --thinking-level max --agent agents/developer.book --context AGENTS.md --test npm run test-for-ptbk-coder`            | [Quickstart step 5](../sections/quickstart.md) |
...
-   `--wait-after-prompt`, `--wait-between-prompts`, `--wait-after-error <duration>` — pacing; durations like `1h`, `30m`, `5s`; errors retry up to 3 times (default wait `10m`).
