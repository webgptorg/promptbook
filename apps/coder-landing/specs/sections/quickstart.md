...
| 4 | Run the queue, then step away      | `RUN_COMMAND`          | Pick a harness and let it work through every ready PRD automatically without task-by-task confirmation; `--test` configures a test or quality command whose failures are retried before the PRD and code are committed together; `--dry-run` previews the queue |
