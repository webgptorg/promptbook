...
-   The window is never wider than the character grid it shows. A layout which offers more width than the widest supported terminal needs — a tablet, a phone in landscape — therefore gets a centered terminal window instead of a window with an empty half.
-   The body is at least as tall as the whole scripted session, so the session never scrolls vertically either and the agent visual **stays fully visible** once the sample has finished. The view must not auto-scroll away from it. When the column next to the terminal is the taller one, the window fills that height and ends in empty terminal background, exactly like a real terminal window that is taller than its output.
-   A terminal row is as tight as a real terminal emulator draws it, because the vertical box drawing characters are about one em tall and a looser row would break every box border into detached segments.
-   The box drawing characters, the shade of the progress bar and the status marks are missing from the latin subset of the terminal font, so they are drawn from the fallback font. That fallback must be a **real monospace font**, never the metric-adjusted fallback `next/font` derives from a proportional font: only a monospace fallback draws them one character cell wide and one row tall, which is what makes the frames of the dashboard boxes connect. A character which is still not one cell wide is pulled back onto the character grid by letter spacing, so every row stays exactly as wide as the cells it is counted as.
