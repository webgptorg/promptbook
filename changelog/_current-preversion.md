-   Showed the `--agent` book's avatar visual as colored ASCII art in the `ptbk coder` terminal dashboard. When `ptbk coder run` / `ptbk coder server` is started with `--agent <agent-book-path>`, the rich terminal UI now renders the same canvas avatar visual the agent has on the website — resolved through the shared web logic (`META AVATAR` commitment first, then `META VISUAL`, then the shared default visual) — instead of the static `PTBK.IO` brand banner; without `--agent` (or when the visual cannot be rendered, for example when the optional `@napi-rs/canvas` module is unavailable or the terminal is non-TTY) the default banner is kept. The conversion is universal and dynamic rather than hardcoded ASCII: the new `renderAvatarVisualAsciiArt` utility rasterizes any built-in avatar visual through the exact same `renderAvatarVisual` canvas pipeline the web uses, and the new `convertImageDataToAsciiArt` utility converts arbitrary RGBA pixels into ANSI-colored half-block (`▀`/`▄`) terminal art with alpha transparency composing into the terminal background, 24-bit true color output, and a 256-color ANSI fallback chosen by the new `$detectTerminalAnsiColorDepth` helper. The `--agent` book is now read once through the new `resolveCoderAgent` helper (replacing `resolveAgentSystemMessage`), which exposes both the compiled system message and the raw agent source used for the visual.

-   Forwarded the `--model` flag from `ptbk coder` commands (`run`, `server`, and the shared `CliAgent` path) to the Claude Code CLI when using `--harness claude-code`, so invocations like `ptbk coder server --harness claude-code --model claude-opus-4-8 --thinking-level max` now bake `claude --model claude-opus-4-8 --effort max ...` into the generated temporary shell script instead of dropping the model. The new option is wired through `ClaudeCodeRunnerOptions`, `ClaudeScriptOptions`, and `buildClaudeScript`, and the resolved model is also surfaced in the prompt status metadata alongside the other harnesses. Existing behavior without `--model` is preserved (the `--model` flag is simply omitted from the shell script).

-   Fixed `ptbk` package invocation parity so `npx ptbk`, project-installed `ptbk`, and dev-installed `ptbk` resolve the same installed CLI version. The `ptbk` package now depends on `@promptbook/cli` directly in addition to the full `promptbook` ecosystem package, and its proxy first delegates to a cwd-local `ptbk` install before falling back to its own package graph. This keeps plain `ptbk` aligned with the project dependency when the fixed proxy is the command reached on PATH, while still supporting the nested `promptbook` dependency layout.
