-   Added the Agents Server **Internal S3** browser at the canonical `/admin/internal-s3` page for superadmins, with directory-style S3 prefix navigation, object links, preserved refresh/prefix URLs, and shared **Storage** tabs linking it to `/admin/files` for superadmins. The S3 browser table, breadcrumbs, object links, and refresh links now live in a shared admin storage component instead of the page itself, keeping the internal-S3 page focused on VPS-wide status/configuration. The former `/superadmin/internal-s3` page now redirects to the admin route instead of carrying a duplicate implementation. The `/admin/files` gallery keeps its server-scoped admin access, now has numeric SQLite affinity for uploaded file sizes, and falls back to stored short URLs when a file row has no storage URL, so standalone file-gallery sorting, display, and file opening stay stable.

-   Added live agent progress to Agents Server durable chat so users can see what the agent is doing while it answers, instead of only the rotating `THINKING_MESSAGES` placeholder. While the local agent runner answers a queued message, it streams the answering coding-harness output (for example Claude Code `--output-format stream-json`) into a live runtime log next to the queued message on the shared local agent folder. The chat worker now reads that runtime log during job synchronization through a new shared `resolveAgentMessageRuntimeActivity` helper, extracts the latest **natural-language assistant narration** (raw tool payloads, JSON envelopes, and other technical details are intentionally ignored), and stores it on the still-running assistant placeholder as the existing progress-card `now` text — which the chat UI already renders in place of the thinking rotation and which then hands off to the final answer as before. The update is best-effort and DRY: a missing or unreadable log, or a harness that does not stream structured text, simply keeps the current `THINKING_MESSAGES` behavior, unchanged runner output never rewrites the chat, and the reader stays `didMutate: false` so it never triggers extra background worker ticks (progress refreshes on the chat's normal detail poll). This covers the local runner that answers chats on standalone VPS installations; the git-backed external runner runs on a separate machine, so it keeps the thinking-message behavior.
