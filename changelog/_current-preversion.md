-   Fixed Agents Server book+chat layout so `<ResizablePanelsAuto/>` fills its parent (`height: 100%` + `min-height: 0`) instead of forcing `100vh`, preventing the chat panels from overflowing the viewport when embedded in other layouts.
-   Recorded Agents Server Playwright integration tests so videos are always written to `/other/integration-tests/videos`, making each `npm run test-e2e` run self-documented for reviewers.
-   Fixed Agents Server header "Create new agent" navigation (which could route to `/agents/[object Object]`) by switching menu creation to the same new-book popup flow used on the homepage, and added equivalent per-folder "View all agents" + "Create new agent" actions in the folder hierarchy (desktop + mobile) using one shared DRY dialog controller.
-   Upgraded shared markdown code-block rendering in Agents Server to use isolated Monaco models per snippet (including chat and other `MarkdownContent` surfaces), with stronger language detection from markdown fence classes and alias normalization so mixed-language code blocks keep correct syntax highlighting without cross-snippet interference.
...
-   Improved Agents Server tool-call streaming by emitting metadata-prefixed frames (so tool call JSON never leaks into the assistant text), teaching RemoteAgent to ignore the metadata, and refreshing the chat UI to display one chip per call with teammate avatars and ⚠️ badges for failed calls.
...
-   Sharpened the voice dictation lifecycle so silence detection fires after ~1 second of quiet, audio tracks are stopped immediately, and the recognition state resets cleanly; the chat input now also surfaces a live status line (preparing, listening, transcribing, error) below the microphone button so it’s obvious what the microphone is doing.
