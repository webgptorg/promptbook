-   Aligned the default Promptbook Coder developer agent and its landing-page terminal sample on the book-declared `ascii-octopus` visual. The landing demo now resolves the avatar from `META VISUAL` instead of repeating its own fallback, and `ptbk coder run --agent` is covered through the CLI handoff.

-   Fixed an intermittent Promptbook Coder verification failure by waiting for a Bash subprocess to close its output streams before collecting its final output.

-   Fixed a Windows-only Agents Server build failure caused by a completed Sentry source-map upload racing with cleanup of its own temporary directory.
