-   Added local per-agent project folders to the Agents Server. Every agent now has its own isolated projects environment: a new `AgentProject` table (backwards-compatible migration) links each project to `Agent.permanentId`, and each project owns one filesystem directory under `.promptbook/agents-server/projects/agent-<id>/<project>` (root overridable via `AGENT_PROJECTS_STORAGE_DIRECTORY`). Agents get always-on chat tools with 100% control over their project folders: `agent_project_list` / `agent_project_create` for managing projects, `agent_project_list_files` / `agent_project_read_file` / `agent_project_write_file` / `agent_project_delete_path` for files, `agent_project_run_script` for running `.bat` / `.cmd` / `.ps1` / `.sh` scripts inside the project folder, `agent_project_git` for running git commands so every project can become and stay a git repository, and `agent_project_link_file` for producing authenticated project/file links that the agent can place into chat. Served links are backed by `/api/agent-projects/<id>` (JSON overview) and `/api/agent-projects/<id>/files/<path>` (file download), readable by administrators and the agent-owning user, with all model-supplied paths confined to the project directory. Administrators can inspect all projects of all agents at **System → Projects** (`/admin/projects`) and browse each project folder including file downloads at `/admin/projects/<id>`. The `/admin/resource-monitor` page now shows a **Project storage** card plus an **Agent Projects** panel with the size of each project and the total size of all projects per agent alongside the existing server disk-space metrics.
