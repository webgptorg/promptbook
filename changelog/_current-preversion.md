...
-   Added anonymous chat persistence in the Agents Server: guests now get `anonymous-<base58>` usernames stored in cookies, the user-chat/memory services reuse the shared identity resolver, and the My chats panel surfaces the same list for anonymous users as it does for signed-in accounts.
