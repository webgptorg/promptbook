-   Improved Agents Server chat switching smoothness by removing route-refresh navigation on sidebar chat changes (URL now updates via `history.replaceState`), preventing duplicate bootstraps/remounts that caused loading blinks and perceived double-loading; also added in-flight guards for chat-select/create actions to avoid accidental concurrent fetches.
