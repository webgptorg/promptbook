-   Improved Agents Server project browsing. The per-agent `/agents/<agentId>/projects` page now shows projects as concise clickable profile cards using README-derived names and descriptions, with a new `/agents/<agentId>/projects/<projectId>` profile page that shows the full README and a GitHub-like current-folder file browser. Project references now reuse shared full/small project item variants across the projects dashboard, agent chat, agent profile, admin projects dashboard, and resource monitor, and project names/descriptions are resolved from README headings and first paragraphs with shared Markdown parsing.

-   Fixed the OpenAI Codex harness login-method probe so Agents Server Codex runs detect the server's ChatGPT login even when the installed `.env` also contains `OPENAI_API_KEY`. The generated Codex runner shell and the standalone VPS installer now run `codex login status` with `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `CODEX_API_KEY` removed from that probe, so API-key fallback stays available only when no ChatGPT Codex login is active.

-   Restricted Agents Server project browsing for anonymous visitors. Public and unlisted agents now show anonymous users only project names and README content on `/agents/<agentId>/projects`, while file listings, paths, sizes, timestamps, and raw project file downloads require a logged-in user. Private-agent projects remain hidden from anonymous users, and the active-agent **Projects** menu item now appears only when the current visitor can see at least one project.

-   Fixed the Agents Server `/admin/chat-history` page so every chat is recorded again. The durable user-chat pipeline (the flow behind the web chat UI and timeout wake-ups) never wrote into the `ChatHistory` table — only the legacy stateless chat, voice, and OpenAI-compatibility routes did, so the admin page stayed empty. Recording now happens at two shared points: `appendQueuedUserChatTurn` records every queued user turn (with request telemetry when available) and `persistUserChatJobTerminalState` records every completed assistant answer from both the local and the git-backed external chat runner, all through the new `recordUserChatMessageInChatHistory` helper built on the existing `createChatHistoryRecorder` (which now works without an HTTP request). A backwards-compatible migration adds nullable `chatId` and `taskId` columns to `ChatHistory`, so each recorded message links to its canonical `UserChat` and its durable chat-completion task. The chat-history table got new row actions: **Create mock** builds a mocked-chat preset (`/system/utilities/mocked-chats`) from the whole recorded chat (shared `createMockedChatPresetFromChatMessages`/`$saveMockedChatPresetFromMessages` utilities), and **Task** opens the new task detail page; the chat view mode has its own **Create mock** button.

-   Added a task detail page `/admin/task-manager/<taskId>` to the Agents Server. It shows everything visible in the `/admin/task-manager` table for one durable background task — status, kind, scope (user/agent/chat), timeline, durations, worker/queue/lease state, attempts, harness run report, errors with downloadable logs, plus the cancel/retry actions and the super-admin live terminal. The data comes from a new `GET /api/admin/chat-tasks/<taskId>` endpoint reusing the whole task-manager listing pipeline (`getAdminChatTaskById`), the presentation building blocks are extracted into `taskManagerTaskPresentation` and the confirm/reason action dialogs into `adminChatTaskActionDialogs`, both shared with the task-manager rows (DRY). Task ids in the task-manager table now link to the detail page, and chat-history rows link to the task which produced the message.

-   Allowed Agents Server super-admins to review the chats of all users. When the environment-backed super-admin enables the admin-only external-chats toggle in the chat sidebar, the chat list now also contains other users' web-UI chats of that agent (reusing the same canonical `UserChat` data), not just frozen API/team chats. Other users' chats open as frozen view-only conversations — the message input, replies, feedback, uploads, and deletion stay disabled (`isUserChatViewOnly` guard on the messages/update/delete routes) — with the frozen banner explaining the chat is from `another user`. For super-admins the banner of such external chats additionally offers a **Create mock** button (saves the conversation as a `/system/utilities/mocked-chats` preset) and a **Chat history** link to `/admin/chat-history`; both appear only when a super-admin views another user's chat.

-   Added `S` and `X` terminal controls to `ptbk coder run` and `ptbk coder server`. `S` skips the active timed wait (`--wait-between-prompts`, `--wait-after-prompt`, or `--wait-after-error`) without affecting prompt execution, while `X` toggles a dynamic end-after-current-prompt request that stops the run at the next prompt boundary. The rich terminal Controls box, plain terminal key listener, server startup copy, internal runner docs, tests, and the `ptbk coder` landing page live terminal/feature copy now show the full `P`/`S`/`X`/`CTRL+C` control set.

-   Updated the Agents Server manGo onboarding wizard so draft generation, Book-language conversion, and the step 4 test/evaluation flow now run through the imported onboarding AI agents instead of local placeholder templating. The client services call new Next API routes backed by `LiteAgent`, preserving the existing wizard UI while generating real Book drafts and live test replies.

-   Added `--min-priority` and `--max-priority` to `ptbk coder run` and `ptbk coder server`, with the existing `--priority` kept as an alias for `--min-priority`. The coder queue now filters runnable prompts by the inclusive priority range while still choosing the highest-priority prompt inside that scope first, validates priority values as non-negative integers, and rejects invalid ranges where the minimum is greater than the maximum. The coder terminal UI, dry-run stats, kanban server prompt classification, VS Code dev scripts, internal runner docs, and `ptbk coder` landing page priority examples were updated to use the new range terminology.

-   Added harness run reports to the Agents Server task details. Every message answered by `ptbk agent-folder run`/`watch` now writes a `<message>.book.report.json` sidecar (shared `AgentMessageRunReport` contract in `src/book-3.0`) next to the finished message book, recording the runner, model, OpenAI Codex login method (server's **ChatGPT account** vs. **API key**, reusing the existing `codex login status` detection), and the reused token-usage estimate. Both the local and the git-backed chat runner synchronization read that sidecar when a chat completes and persist it into the durable job, so `/admin/task-manager` task details now show a **Runner** row (for example `codex (gpt-5.2-codex)` with `via ChatGPT account`) and a **Usage** row with the run price and token counts. The price/login-method formatting is shared with the prompt-runner status lines (`formatUsagePrice` moved to `src/execution/utils`, `codexLoginMethod` moved to `src/book-3.0`).

-   Added **agent projects** to the Agents Server. Every agent now has its own `projects/` folder inside its local agent directory, where each subfolder is one project — an isolated environment the agent has 100% control over: it can create, read, and write files there, run scripts, keep persistent data between conversations, and turn any project into a git repository. The prompt of every coding harness now contains a `## Projects` section (built by `buildAgentProjectsPromptSection` and shared by `ptbk agent-folder run`/`run-once`/`run-multiple` and `ptbk agent exec`/`CliAgent`) teaching the agent to create a project when asked to build something (for example a website) and how to link a project or any of its files into the chat. On server-managed agent folders the links point to the new raw project file route `/agents/<agentId>/projects/<projectName>/files/<filePath>`, which follows agent visibility rules and serves active content (HTML/SVG/XML) with a sandboxing `Content-Security-Policy` so hosted pages can render and run scripts but never read Agents Server cookies. There is a new per-agent **Projects** dashboard at `/agents/<agentId>/projects` (added to the agent view dropdown in the header next to Timeouts) and a new admin **Agent Projects** dashboard at `/admin/projects` listing all projects of all agents with their folders (linked from the System menu); both dashboards reuse the same `AgentProjectsBoard`/`AgentProjectCard` components and are interlinked. The `/admin/resource-monitor` page now shows the size of the projects of each agent and the total size of all agent projects alongside the server disk usage.
