-   Added support for `ANTHROPIC_API_KEY` environment variable as the standard name for the Anthropic API key. The previous `ANTHROPIC_CLAUDE_API_KEY` is kept for backward compatibility. When both are set, `ANTHROPIC_API_KEY` takes precedence. The Agents Server VPS configuration UI now also lists `ANTHROPIC_API_KEY` as a supported environment variable.

-   Added macOS desktop app for editing `.book` files at `apps/book-editor-macos`. The app is built with Electron + Vite + React and reuses the existing `<BookEditor/>` component from `@promptbook/components`, giving it the same Monaco-based syntax highlighting and editing experience as the web editor. A native macOS menu provides File → New / Open… / Save / Save As… operations, file associations for `.book` and `.book.md` are declared in the `electron-builder` config so macOS registers the app as the default editor, and the hidden-inset title bar lets the window chrome blend with the macOS traffic-light buttons. To build: `cd apps/book-editor-macos && npm install && npm run package`.

-   Added VSCode extension for the Book language at `apps/vscode-extension`. The extension provides syntax highlighting (TextMate grammar), snippets, and language configuration for `.book` and `.book.md` files. Both the grammar (`syntaxes/book.tmLanguage.json`) and the snippets (`snippets/book.json`) are dynamically generated from the commitment registry via `npm run generate-vscode-grammar`, so they stay in sync with newly added commitments automatically. Multi-line commitment blocks (NOTE/NOTES/NONCE, TODO, FROM/IMPORT/TEAM), fenced code blocks, template parameters, and agent references are all highlighted. Publishing to the Visual Studio Code Marketplace is automated in the `publish-vscode` job of the GitHub Actions publish workflow and triggers on every new version tag. The NOTE/TODO/agent-reference commitment-type constant lists were extracted to a shared `BookEditorCommitmentConstants.ts` file so both the `<BookEditor/>` Monaco tokenizer and the VSCode extension generator share a single source of truth.
