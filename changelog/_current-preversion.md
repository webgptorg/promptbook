...
-   Added `BOOK_ALLOW_DOCUMENT_UPLOADS` and `BOOK_ALLOW_IMAGE_UPLOADS` metadata toggles that gate drag-and-drop/file-picker/camera uploads inside `/agents/[agentName]/book` and `/agents/[agentName]/book+chat`; the new `BookConfiguration` helper keeps the book editor controls in sync with the flags, and both routes now hide the upload/camera UI plus stop wiring the upload handler when a flag is disabled (defaults stay `true`).
