-   Added the Agents Server **Internal S3** browser at the canonical `/admin/internal-s3` page for superadmins, with directory-style S3 prefix navigation, object links, preserved refresh/prefix URLs, and shared **Storage** tabs linking it to `/admin/files` for superadmins. The former `/superadmin/internal-s3` page now redirects to the admin route instead of carrying a duplicate implementation. The `/admin/files` gallery keeps its server-scoped admin access and now has numeric SQLite affinity for uploaded file sizes, so standalone file-gallery sorting and display stay stable.
