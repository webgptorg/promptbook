...
-   Reworked `USE EMAIL` for Agents Server to support wallet-backed outbound SMTP sending:
    ...
    -   `send_email` now persists outbound emails through a reusable two-step pipeline (`Message` creation + per-provider `MessageSendAttempt` logging), returns structured send status/attempt results, and emits wallet-credential-required payloads with user instructions when SMTP credentials are missing.
    -   Added SMTP provider implementation (`nodemailer`) and generalized chat wallet-popup handling so wallet credential prompts are not limited to `USE PROJECT`.
-   Reworked Agents Server chat synchronization to be browser-independent for user chats: active chat requests now carry a dedicated chat-sync id, `/agents/:agentName/api/chat` persists in-progress assistant output and tool calls directly into `UserChat` during streaming (and continues persisting even when the browser stream disconnects), and chat clients now poll every 5 seconds and hydrate local chat state from persisted messages so refreshes, multiple windows/devices, and offline-then-online reconnection automatically converge without manual reloads.
