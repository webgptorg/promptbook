-   Updated Agents Server agent archive transfer on the agents listing: the visible `Export` action is now `Download`, the separate `Import` button was removed because admins can import by dropping `.book` or ZIP files, ZIP drops on a folder card now import agents into that folder, and downloads started from an open folder now include only that folder subtree instead of the whole server.

-   Fixed standalone VPS Agents Server installs that accepted an OpenAI API key but still left the local agent runner without the `codex` binary. The installer already wrote `OPENAI_API_KEY` and `PTBK_OPENAI_CODEX_USE_API_KEY=1`, but first-time non-interactive installs skipped all code-runner CLI dependency setup, so chats later failed with `codex: command not found` and exhausted the local runner retry limit. `configure_code_runner_for_initial_installation` now detects the `openai-codex` + API-key configuration even in non-interactive mode, installs `@openai/codex`, and runs the existing API-key authentication configuration path before starting the server. Interactive installs and other unauthenticated non-interactive runner setups keep their previous behavior.

-   Replaced the multi-section "Working on your request" placeholder card with a single, real-time activity line in Agents Server durable chats so users see what the harness is doing **right now**, one thing at a time, instead of a verbose scripted checklist of upcoming steps.

    -   `createUserChatProgressCard` in `apps/agents-server/src/utils/userChat/userChatProgressCard.ts` no longer emits the fixed `Working on your request` title, the 4-item pending/completed checklist (`receive-message` / `prepare-agent` / `check-capabilities` / `create-response`), or the `What I'll Do Next:` section for the lifecycle phases driven by the worker. Each phase now carries a single short `now` status — `Waiting for a worker to pick up your message...`, `Reading your message and 2 attachments...`, `Loading agent **{name}**...`, `Checking 3 tools (web browser, calendar), email access...`, `Calling the language model via OpenAI...` — that is enriched with the real signals the worker has already resolved at that point (agent name, attachment count, tool highlights, integrations, provider, fresh-runtime hint). The now-unused `composeUserChatProgressPhaseText` / `USER_CHAT_PROGRESS_PHASE_CHECKLISTS` / `USER_CHAT_PROGRESS_ITEMS` / `USER_CHAT_PROGRESS_TITLE` constants and the `describeKnowledgeHint` helper are dropped, and the composer is renamed `composeUserChatProgressPhaseNow` to match its single-output signature.
    -   `createProgressCardChecklistMarkdown` in `src/book-components/Chat/Chat/createProgressCardChecklistMarkdown.ts` now detects when a card carries only a live `now` value (no title, no items, no `next`) and renders it as a single italic line (`_status..._`) instead of the verbose `**What I'm Doing Now:** status` label format, so the harness phases display as a clean ChatGPT-style live status. Cards populated by the LLM through the `agent_progress` tool with structured `items`, `title`, or `next` keep their full checklist layout unchanged, so agent-driven progress with multiple bullets still renders the same way.
    -   `userChatMessageLifecycle.test.ts` updated to assert the new single-status shape (`now: 'Waiting for a worker to pick up your message...'`, empty `items`, no `title`, no `next`) for the queued assistant placeholder.
