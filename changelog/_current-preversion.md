-   Added a real-time read-only CLI terminal for every background task in the Agents Server `/admin/task-manager`. Each task row now shows **Copy log** / **Download log** buttons for its post-mortem log, and — for the super-admin only — a **Terminal** button that opens a live xterm dialog streaming only that task's own console output instead of the whole Agents Server terminal. Durable chat completions, chat timeouts, and browser previews mirror their console output into a bounded in-memory per-task terminal log through a shared `AsyncLocalStorage`-based console capture (`runWithTaskTerminalCapture`), standalone VPS self-update tasks live-tail the persisted installer log file instead because the updater runs as a detached process, and the new super-admin-only `GET /api/admin/chat-tasks/{taskId}/terminal` endpoint serves both the JSON snapshot and the Server-Sent-Events live stream by reusing the existing shared admin terminal SSE helper, browser hook, and xterm renderer.

-   Refactored `$registeredLlmToolsMessage` so the exported file is now a small facade. `.env` origin state, provider register snapshots, provider status assembly, and markdown/terminal rendering now live in focused private modules under `$registeredLlmToolsMessage/`, while the exported helpers and observable provider summary output remain unchanged.

-   Refactored `validatePipeline` so `validatePipeline.ts` is now a small validation facade. Pipeline context creation, metadata validation, collection-shape validation, parameter validation, task validation, and dependency-resolution validation now live in focused private modules under `validatePipeline/`, while the exported validation API and observable validation behavior remain unchanged.

-   Refactored `ptbk agents-server start` foreground launcher so `startAgentsServer.ts` is now a small orchestration facade. Path and `.env` resolution, child environment preparation, log streams, supervisor state, Next process handling, runtime preparation, local runner limits, internal-route error formatting, and the local user-chat worker pump now live in focused private modules under `startAgentsServer/`, while the exported CLI helper API and observable startup behavior remain unchanged.
...
-   Added garbage collection of old installed Agents Server versions on the standalone VPS so multi-gigabyte release checkouts in `PTBK_RELEASES_DIR` can no longer exhaust the disk. The installer's `self-update` now runs a shared `garbage_collect_promptbook_releases` step for both manual and automatic updates — once before cloning the new checkout (freeing disk space and removing stale `.install-*` staging directories from interrupted runs) and once after the nginx switch, replacing the previous unconditional deletion of the just-replaced release. The newest `AGENTS_SERVER_GC_KEEP_VERSIONS` versions (default `3`: the current version plus two previous ones, editable from `/admin/environment`) are kept in total, and the currently deployed version is never deleted. `/admin/update` now shows an **Installed versions** card listing every installed version (current one badged) with per-version manual deletion through the new `DELETE /api/admin/update/versions` admin API, which refuses to delete the current version, unknown names, or anything while a self-update job is running.

-   Added an Agents Server super-admin resource monitor at `/admin/resource-monitor`. The page reads current CPU load, process CPU sample, memory, disk, and Linux network traffic counters, shares one threshold resolver for resource-pressure warnings, and surfaces a warning indicator on the System / Super Admin / Resource monitor menu path when CPU, memory, or disk space is low.

-   Updated the `ptbk coder` landing page live terminal so it replays real `ptbk coder run` dashboard snapshots instead of the old scripted fake work log. The hero sample now types a representative limited run command, renders Session / Current task / Live output / Controls boxes through the shared `buildCoderRunUiFrame` renderer, and resolves the `AsciiOctopus` agent visual through the same shared avatar-to-ASCII path used by the CLI and Agents Server visuals.
