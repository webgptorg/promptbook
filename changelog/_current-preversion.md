-   Refactored `src/book-2.0/agent-source/createAgentModelRequirementsWithCommitments.ts` into smaller focused internals without changing requirement-compilation behavior:
    ...
    -   Kept the existing DELETE/GOAL filtering rules, reference-resolution fallbacks, TEAM profile pre-resolution, imported-file handling, MCP extraction, example interaction rendering, and inline-knowledge upload fallback behavior unchanged.
...
-   Refactored the Agents Server chat-history Playwright coverage into focused support modules without changing the covered behavior:
    ...
    -   Moved chat-history-specific URL parsing and delayed-request interception behind a dedicated support facade so the main spec now focuses on regression scenarios.
