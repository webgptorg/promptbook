-   Updated Agents Server project references in chat. Project chips are no longer shown as a persistent chat header strip, visible projects are reachable from the agent context menu via `/agents/<agentId>/projects`, and known `[[project-name]]` markdown references in chat messages now render as project profile chips.

-   Added super-admin browser VS Code access for Agents Server agent projects. Project detail pages now show a **VS Code** action and support the GitHub-style `.` shortcut, starting a code-server workspace rooted at the selected project with the current light/dark theme applied. Standalone VPS installs now install `code-server` and add an authenticated Nginx WebSocket proxy under `/agent-project-vscode/`, with an Agents Server auth subrequest resolving the local project editor port so the full integrated terminal remains restricted to super admins.
