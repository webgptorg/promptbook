-   Fixed the visual of the `ptbk coder run` terminal dashboard, which always looked like the default octopus no matter which visual the agent book selected. The visual really was taken from the agent book — `agents/coding/developer.book` declares `META VISUAL ascii-octopus` and that value did reach the terminal — but every built-in visual was rasterized onto a canvas and converted back into colored half-block characters, so `AsciiOctopus`, which _is_ made of ASCII glyphs, was averaged down into exactly the same blob silhouette as the pixel-based octopuses. Visuals which are made of characters now paint the terminal character grid directly instead: `AvatarVisualDefinition` gained an optional `renderTerminalText` renderer, `AsciiOctopus` samples its morphing octopus field into one deterministic glyph grid which its canvas renderer and its new terminal renderer share, and the shared terminal avatar renderer prefers that grid over the raster pipeline. The `ptbk coder run` dashboard therefore now opens with a real ASCII octopus drawn from `@`, `#`, `%`, `*`, `+`, `=`, `o`, `0`, `|`, `!`, `\` and `/`, with its eyes and mouth legible, and rendering it no longer needs the optional `@napi-rs/canvas` module at all. Pixel-based visuals are untouched and keep using the existing half-block rasterization, and the ANSI color-code generation both terminal renderers need is now shared instead of living inside the half-block converter.

-   The default developer agent which `ptbk coder init` creates now declares `META VISUAL ascii-octopus`, exactly like the Promptbook Developer agent of this repository, so a freshly initialized project gets the same ASCII octopus in its `ptbk coder run` dashboard. The live terminal of the [`ptbk coder` landing page](apps/coder-landing) resolves its visual from that agent book as well, instead of from a landing-specific fallback visual.

-   Fixed the Agents Server production build, which stopped compiling once project cards started reading their title and icon from the project `index.html`. The icon links are matched by an attribute selector, so TypeScript types them as generic `Element`s, while the icon-relation check declares an `HTMLLinkElement` parameter; the element type is now declared on the query itself, as `<MarkdownContent/>` already does. The parsed titles and favicon paths are unchanged.
-   Improved Agents Server project cards so their shared metadata uses a README heading or the title of index.html, then displays a detected project favicon with deterministic initials as the fallback. The agent dashboard, admin dashboard, and compact project references now reuse the same title and icon resolution.
-   Added **agent goal chats** to the Agents Server. Every agent now has exactly one goal chat — the thread the agent keeps with itself to plan work towards its goal, rather than a place where a person talks to it. It is stored as one ordinary chat row with the new `AGENT_GOAL` source and a chat id derived from the agent permanent id (`goal-<agentPermanentId>`), so the singleton is enforced by the primary key, concurrent requests converge on the very same row, and any layer holding only a `chatId` — SQL task queries, admin task rows, sidebar rows — recognizes a goal chat without an extra lookup. The chat is created lazily on first use and its stored owner is the agent owner (with the same fallback ordering the ownership migration uses for unowned agents), because a chat row needs a `userId` even though the chat belongs to the agent; the goal chat is therefore looked up by its deterministic id alone and survives a change of agent ownership. Whoever may read the agent source book may read the goal chat, which is one shared rule so the two surfaces cannot drift apart, and when visible the goal chat appears among the agent's other chats pinned above all of them with its own goal icon in both the expanded and the collapsed sidebar. Only the agent writes into it, so it is read-only: the chat surface renders no input area and shows a notice explaining what makes this thread special. The agent leaves a note in its goal chat when it is created, when its book is modified, and whenever it plans or cancels a message for the future; leaving such a note is best-effort, so creating or saving an agent can never fail because of its goal chat.

-   **Breaking:** Removed the `/agents/:agentName/timeouts` page of the Agents Server and moved its purpose into the [goal chat](specs/chat/goal-chat.md). Scheduled wake-ups are the agent's **planned messages**, and the agent's own thread is now the single place where they are visible: the goal chat lists every planned message of the agent together with the time it is planned to be executed and allows cancelling any of them at any time. Because a planned message belongs to the agent rather than to one user, the listing spans every chat and every user of that agent, so `GET /agents/:agentName/api/timeouts` and `DELETE /agents/:agentName/api/timeouts/:timeoutId` are now agent-scoped and gated behind goal-chat access. The timer dashboard's extras that had no place in the agent's own thread are gone with the page: `PATCH /agents/:agentName/api/timeouts/:timeoutId`, `POST /agents/:agentName/api/timeouts/actions` and their browser clients were removed. The **Timeouts** entry of the agent menu was replaced by a **Goal** entry pointing at the new `/agents/:agentName/goal` route, which resolves the singleton goal chat and hands over to the shared chat surface instead of duplicating the chat UI.

-   Marked agent-initiated background work in the Agents Server **Task Manager**. A chat completion or timeout that runs in an agent's goal chat keeps its normal kind, so it is still listed among the chat completions, and only carries a new `isAgentGoalTask` flag rendered as a **Goal chat** badge on the task row and the task detail page. The flag is derived in one place from the goal-chat id, so it needs no schema change and stays correct in the direct-SQL as well as the SQLite fallback task query.
