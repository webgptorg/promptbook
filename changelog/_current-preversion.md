-   Added an Agents Server internal S3 file browser to `/admin/internal-s3`, showing folder-style object listings under the configured path prefix with breadcrumbs, parent navigation, object sizes, timestamps, and direct object links. Superadmins now get shared route tabs between `/admin/internal-s3` and `/admin/files`, while normal admins still use `/admin/files` as the current-server upload gallery. The `/admin/files` gallery now loads file rows and agent labels through separate simple queries, fixing the blank filenames, `NaN KB` sizes, and missing dates caused by relation selects in the local SQLite Supabase adapter.

-   Added custom project domains to the Agents Server. `/admin/projects` now lets admins assign or clear a full custom domain for each agent project, including root domains and deeper hostnames such as `client.website.example.com`; project runtime startup reuses the custom assignment instead of forcing `<project>.<server-domain>`, running runtime records refresh their public URL after an admin change, and `/admin/servers` now lists these assignments as project domains with custom-domain badges.

-   Changed `ptbk coder` so a finished prompt reports **usage step by step** instead of one lumped total. The completed `[x]` line now reads ``[x] by Claude Code `claude-opus-4-8` - Implementation $8.01 6 hours; Testing 1 hour`` — each coding step (`Implementation`, `Fixing`) carries its own price and duration and each verification step (`Testing`) carries only its duration. `runPromptWithTestFeedback` records each step as it runs and `markPromptDone` renders them through a shared `formatCoderRunSteps` helper. The `ptbk coder` landing website gained a matching **Cost and time, step by step** feature card.

-   Fixed Agents Server project listings so `/admin/projects` and the project storage section of `/admin/resource-monitor` only aggregate projects whose owning agent exists in the current server database. Local project folders left by another server on the same VPS are no longer shown as `Unknown agent`, and the dashboard now counts “Agents with projects” against current-server agents instead of every project folder under the VPS-wide agent root. Project subdomain assignments shown on `/admin/servers` are also keyed by server domain, so the same agent/project identifiers on different server domains keep separate project subdomain records.

-   Made Agents Server agent ids **case-insensitive** so `/agents/doQMRg82izNfJa/chat`, `/agents/DOQMRG82IZNFJA/chat`, and `/agents/doqmrg82iznfja/chat` all resolve to the same agent (a mis-cased id now redirects to the canonical URL instead of returning a 404). Agent lookups by id or name share a single Promptbook Engine helper `buildAgentNameOrPermanentIdFilter` that matches the permanent id with a case-insensitive `ilike` while agent names keep their exact matching; SQL `LIKE` wildcards (`%`, `_`, `\`) in the lookup value are escaped so identifiers are still matched literally. The remaining in-memory identifier comparisons (owner-scoped agent disambiguation, the route-target collection fallback, and custom-domain resolution) now go through a shared `isSameAgentPermanentId` helper.

-   Fixed the Agents Server `/admin/task-manager` **All** history view so tasks are ordered by finished time, with the newest finished task on top. The `All` view previously ordered rows by last-update time, which let a self-update that finished long ago sit above chat completions that finished more recently. The default `All` ordering now reuses the shared task timeline timestamp (which resolves to `finishedAt` for terminal tasks) in both the SQLite comparator and the PostgreSQL `ORDER BY`, while still-active tasks keep their start/queue placement instead of dropping to the bottom. An end-to-end `getAdminChatTasksResponse` regression test now covers the reported screenshot case where injected self-update rows are merged with database chat rows, asserting that a self-update finished 17 hours ago sinks below chat completions finished 7 and 10 hours ago instead of staying pinned on top.
