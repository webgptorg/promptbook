...
-   Added a stop-streaming control to the Agents Server chat (powered by a new `stopCurrentChatStream` hook in `LlmExecutionTools`), letting users cancel long responses without errors; `RemoteAgent` now aborts the underlying `/api/chat` fetch and returns the partial content so the assistant message stays visible instead of failing.
