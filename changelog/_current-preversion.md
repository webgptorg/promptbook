...
-   **New:** Agents Server `/admin/chat-feedback` page with a searchable, filterable, and paginated table of chat feedback entries stored in `ChatFeedback`, including per-entry deletion and agent filters, reusing the same smart-table and Supabase patterns as chat history to keep things DRY. [✨💭]
-   **New:** Admins can clear all chat feedback for a specific agent from both the `/admin/chat-feedback` page and each `/agents/[agentName]` page via a dedicated maintenance action, sharing the same API surface for bulk deletes.
