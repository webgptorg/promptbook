-   Changed the Agents Server built-in defaults so fresh installations start with the **Light** theme and the **Article** chat visual mode instead of the previous **System** theme and **Bubble** chat visual mode. `DEFAULT_THEME_MODE` in `apps/agents-server/src/constants/themeMode.ts` and `DEFAULT_CHAT_VISUAL_MODE` in `apps/agents-server/src/constants/chatVisualMode.ts` now point at `LIGHT` and `ARTICLE_MODE` respectively, so the `metadataDefaults` fallbacks surfaced by `getMetadata` reflect the new defaults. The existing `2026-05-2600-default-theme.sql` seed migration also now inserts `LIGHT` (and uses `LIGHT` as its invalid-value fallback) for new installations; already-migrated servers keep whatever `DEFAULT_THEME` value they had, so administrator customizations are preserved.

-   Changed Agents Server KNOWLEDGE source previews for non-embeddable web pages from a one-time screenshot into a live browser-backed preview. The citation modal now opens a shared Playwright browser session through `/api/page-preview/live`, streams JPEG frames into the existing preview surface, and forwards click, wheel, and key interactions through `/api/page-preview/interact` using the same session id, so pages that block iframes remain inspectable without freezing on the first captured frame. The existing page-preview auth and SSRF URL checks are now shared across the check, screenshot, and live-preview routes.
...
-   Removed the placeholder **"Working on your request"** title from Agents Server durable chat progress cards so the response no longer shows that generic heading between the `THINKING_MESSAGES` rotation and the final answer. The phase-based progress card (`createUserChatProgressCard`), the out-of-process runner progress card (`createUserChatRunnerProgressCard`), the in-process harness progress card (`createUserChatHarnessProgressCard`), and the `agent_progress` tool's default title fallback no longer set the literal `'Working on your request'` heading; the existing `now`/`next`/`items` real-state markdown still renders unchanged and the title field stays optional so agents can still supply their own meaningful heading through `agent_progress`.
