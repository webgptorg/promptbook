...
-   Fixed Agents Server chat feedback saving so the toast now reports the actual server response (green on 201, red with the returned message on errors), sits above the sticky header/right menu, and reuses the shared rating hook to keep the UI DRY.
