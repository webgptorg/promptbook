...
-   Improved OpenAI Assistant caching for agents:
    -   Implemented persistent caching of OpenAI Assistant IDs using the `preparedExternals` field in the Agent database table
    -   Added hash-based validation to detect when assistant requirements have changed
    -   Assistants are now reused across agent restarts and server deployments when requirements haven't changed
    -   Significantly reduced agent preparation time by avoiding unnecessary assistant recreation
    -   Added callback mechanism to persist assistant cache updates to the database automatically
    -   Follows DRY principle by centralizing caching logic in `AgentLlmExecutionTools`
