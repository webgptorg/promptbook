...
-   Refactored the Book editor Monaco upload hook so `useBookEditorMonacoUploads` is now a small orchestrator. The shared upload types, the scheduled-timer helper, Monaco placeholder insertion and queued-item creation, editor placeholder/replacement synchronization, upload-item state and stats aggregation, progress-update debouncing, the upload concurrency/retry/pause queue, and completed-upload auto-clearing now live in focused private modules under `useBookEditorMonacoUploads/`. The exported `useBookEditorMonacoUploads` hook and its returned API, the re-exported `UploadItem` and `UploadStats` types, the Monaco placeholder/decoration behavior, and the observable upload progress, pause/resume, and editor replacement behavior remain unchanged.
