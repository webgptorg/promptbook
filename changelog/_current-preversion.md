-   Hardened the standalone VPS Agents Server self-update (`/admin/update`) so a failing host-software installation can never brick future updates again. Dependencies — including host software newly required by the target release, such as `code-server` — are still ensured on **every** self-update through the shared `apply-dependencies` entry point of the freshly checked-out release (the same functions a fresh `install.sh` run uses), but the dependency step is now non-fatal: when one installation fails (network hiccup, apt failure, download outage) the self-update warns, finishes, and simply retries the installation on the next update, while hard requirements keep failing the update where they are really needed (`npm ci`, the Agents Server build). Background: the update is always orchestrated by the **already-installed** release's `install.sh`, so a server whose installed script still installed `code-server` through `npm install --global` kept aborting every update with `Please pass --unsafe-perm to npm to install code-server` regardless of the fixes already on `main` — such servers are recovered by running the current `install.sh` once over SSH, which performs exactly the standard self-update (same code path as the admin panel) and repoints future panel updates at the fixed script. Also realigned the `install.sh` regression tests with the per-domain certificate and generated-nginx-site architecture so the guard suite runs green again.

-   Fixed standalone VPS Agents Server project domains and SSL certificates so one broken domain can never take the whole server offline. Previously `install.sh apply-domains` (run automatically when an agent project starts or a server is added in `/admin/servers`) rewrote the nginx site with HTTP-only server blocks — wiping the certbot-installed HTTPS blocks — and then requested **one** Let's Encrypt certificate bundling the main domain with every project subdomain; a single subdomain with missing DNS failed the whole request (and a non-resolving domain even aborted the script inside the DNS pre-check under `set -e`), leaving port 443 dead for all domains including the primary one. The installer now issues an **isolated certificate per domain** (`certbot certonly --webroot --cert-name <domain>`, served from a static ACME webroot so certbot never edits the nginx configuration) and generates the nginx site deterministically from the certificates already on disk: every domain with a certificate gets its own HTTPS server block plus HTTP→HTTPS redirect, domains without one stay reachable over plain HTTP until their DNS is ready, and the raw-IP default server always keeps the Agents Server reachable. The site configuration is written with backup, `nginx -t` validation, and automatic rollback; a renewal deploy hook reloads nginx after certificate renewals; per-domain certificate failures only warn (listing the failed domains and the required DNS record) so project starts, `/admin/servers` changes, and self-updates always finish — self-update now also refreshes the nginx and SSL configuration so already broken servers heal on their next update. Covered by `other/vps/test-nginx-config-generation.sh` and verified live on a broken VPS.

-   Added per-prompt model and harness requirements to `ptbk coder`. A prompt can now pin the model, harness, or a whole model family it needs directly in its `[ ]` status line by naming it in backticks — for example `` [ ] use `gpt-5.5`  ``, `` [ ] !!!! `gpt`  `` (combined with a priority), or `` [ ] use `github-copilot` !!!!!  ``. Each backtick token is matched against the normalized names of the currently running model and harness using the shared `normalizeToKebabCase` helper (so `gpt-5.5` → `gpt-5-5` and `claude-opus-4.8` → `claude-opus-4-8`); a token matches when it forms a whole dash-delimited segment run of the name, which lets family tokens such as `gpt`, `claude`, `claude-opus` and `opus` match `gpt-5-5` / `claude-opus-4-8` while unrelated tokens do not. A prompt runs when at least one of its tokens matches the running model or harness and is skipped otherwise; prompts without any token keep running on every runner as before. The matching is fully dynamic (no hardcoded model or harness list) and is applied during prompt selection through a new `RunnerFilter` (`scripts/run-codex-prompts/prompts/runnerFilter.ts`) threaded through `listRunnablePrompts`, `findNextTodoPrompt` and `listUpcomingTasks`. The `ptbk coder` landing page gains a matching **Pin a model or harness** advanced-feature card.
...
-   Added a super-admin **Internal S3** page at `/admin/internal-s3` to the Agents Server for managing the bundled self-contained S3 (VersityGW) object storage. Linked from the System → Super Admin menu (super-admin only), the page shows the storage configuration read from the environment — storage mode, endpoint, bucket, region, path prefix, public URL, force-path-style, access key id, whether the secret access key is configured, on-disk data directory, systemd service name, and service port — while never exposing the secret access key itself. When the self-contained S3 is the active, fully configured storage it also runs a time-boxed live probe (a bounded `ListObjectsV2` enumeration) that reports reachability, response latency, stored object count, and total object size; for external-S3 / Vercel Blob deployments or half-configured setups the live check is skipped with an explanation. New read-only helpers under `apps/agents-server/src/utils/internalS3/` reuse the existing `isSelfContainedS3StorageSelected` storage-mode check and the shared `formatResourceBytes` formatter.

-   Fixed the Agents Server chat feedback stars so unpicked stars no longer all appear yellow. The rating UI now renders a text-presentation star (`★`) instead of the color-presentation emoji (`⭐`); color emojis are painted by the platform emoji font and ignore the CSS `color`, which made the already-correct active/inactive colors (gold vs. grey) invisible in both the per-message rating control and the rating modal.
