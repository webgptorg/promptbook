-   Added full Agents Server Playwright integration coverage for core user flows (anonymous access, admin login/logout, Documentation/System navigation, and protected API authorization), introduced a deterministic in-memory Supabase mock backend for reliable e2e runs, and extended the Agents Server CI workflow to execute the new browser-based integration suite.
