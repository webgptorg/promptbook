-   Added super-admin browser VS Code access for Agents Server agent projects. Project detail pages now show an **Open VS Code** action and support the GitHub-like `.` shortcut; the launcher starts a project-root `code-server` session, matches the current light/dark theme, proxies it through an authenticated same-origin route, and keeps the integrated terminal rooted at the viewed project with the same server permissions as the Agents Server process.
