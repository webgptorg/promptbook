...
-   Added standardized expandable project chips to Agents Server chat. Agent `[[project]]` references and legacy Markdown links to project files now show their running state and open a compact menu with **Open the project in a new tab** and **Open the project page in a new tab** actions; the first action uses the public runtime URL instead of an internal `/files/...` URL, while profile-preview, private-mode, and durable chats reuse one runtime-enriched project reference model and shared markdown-chip renderer.
