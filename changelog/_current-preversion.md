...
-   Fixed Book editor instant teammate creation for unresolved `TEAM` references in Agents Server: creating `Create {name}` now refreshes reference diagnostics immediately (no stale resolver cache), ignores soft-deleted duplicates when checking existing agents, keeps the same-folder creation behavior, and updates the missing-teammate panel reliably right after creation.
