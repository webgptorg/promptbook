...
-   Added a "More" dropdown inside the agent navigation hierarchy so every agent-specific page (history, usage, integration, system message, export, etc.) is reachable without the context menu, and removed the redundant Back-to-Agent arrow from those pages.
