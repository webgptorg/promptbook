...
-   **Fix:** Agents Server `/admin/metadata` page no longer shows both the default and a custom value for the same metadata key immediately after saving. The UI now hides the default row whenever a stored value exists for that key, without requiring a full page reload.
...
-   **New:** Agents Server now auto-federates the core Promptbook server (`https://core.ptbk.io/`) using `FEDERATED_SERVERS` metadata plus a shared config list in `servers.ts`, with logic centralized in `getEffectiveFederatedServers` to keep things DRY.
