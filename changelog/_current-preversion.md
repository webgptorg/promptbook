...
-   Expanded the Agents Server **standalone VPS self-update** super-admin page (`/admin/update`) with deeper deployment-drift insight and an arbitrary-ref custom target:
    ...
    -   Reordered the target-environment tiles to Live / `main`, Preview / `preview`, Production / `production`, LTS / `lts`, Custom. The legacy `production` fallback in `resolveVpsSelfUpdateEnvironment` is preserved via the new explicit `getDefaultVpsSelfUpdateEnvironment` helper.
    -   Added a new **Custom** target option with a commit picker that lists candidate commits from the managed checkout (`git log --all`) with branch/tag annotations, free-text filtering across hash/subject/author/branch/tag, and authored-date range filters. Submitting an arbitrary commit, tag, or branch surfaces an unstable-build warning whenever it is not pointed at by a release tag.
    -   Added a collapsed **Advanced** panel that lets the super admin override the upstream repository URL away from the default `https://github.com/webgptorg/promptbook.git`. The new value is validated as a public `https://` git URL and persisted to the standalone-VPS `.env` (`PROMPTBOOK_REPOSITORY_URL`) so subsequent installer runs reuse the fork.
    -   Extended `apps/agents-server/src/utils/vpsSelfUpdate.ts` with the new overview fields (`commitsBehindCount`, `currentCommitDate`, `latestRemoteCommitDate`, `originRepositoryUrl`, `isOriginRepositoryDefault`, `defaultOriginRepositoryUrl`), a `listVpsSelfUpdateCandidateCommits` helper that fetches branches/tags before listing commits, and a new `VpsSelfUpdateStartRequest` shape covering the optional `customRef` and `originRepositoryUrl` overrides.
    -   Added the `/api/admin/update/commits` super-admin endpoint backing the picker (super-admin-only, filterable via `search`, `after`, `before`, and `limit` query params).
    -   Taught `other/vps/install.sh self-update` to accept a new `--ref <sha|tag|branch>` flag that bypasses the production/main/preview/lts whitelist, falls back from a shallow branch clone to a full clone + detached checkout for arbitrary refs, and now picks up `PROMPTBOOK_REPOSITORY_URL` from `.env` during runtime configuration loading.

-   Changed the Agents Server footer's "Made with ❤️ in Europe" line to show a per-deployment emoji derived from `NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA`, so users can recognize a server update at a glance by spotting that the footer emoji changed:

    -   Added `apps/agents-server/src/components/Footer/getCommitFooterEmoji.ts`, a pure utility that hashes the commit SHA with a small djb2-style string hash and maps it to one entry from a curated list of positive/neutral emojis (hearts, cute animals, plants, fruits, sweets, celebratory symbols). When the commit SHA is missing (for example during local development outside Vercel) it falls back to the original `❤️`.
    -   Replaced the hardcoded heart in `footer.madeInCzechRepublic` (English and Czech translations) with a `{emoji}` placeholder and passed the derived emoji through the existing `t(key, variables)` interpolation in `apps/agents-server/src/components/Footer/Footer.tsx`.
    -   Added unit tests in `apps/agents-server/src/components/Footer/getCommitFooterEmoji.test.ts` covering the missing-SHA fallback, determinism for the same SHA, and variation across different SHAs.

-   Fixed stored XSS vulnerability in the Agents Server **Custom JavaScript** admin feature. The previous sanitization in `apps/agents-server/src/app/layout.tsx` only escaped closing `</script>` tags inside the admin-provided script before rendering it with `dangerouslySetInnerHTML`, which is trivially bypassed using HTML entities, event-handler attributes, or other encoding tricks. An admin (or an attacker who compromises the database or an admin account) could therefore inject arbitrary JavaScript that executed in every visitor's browser, enabling session hijacking, credential theft, and redirection.

    -   Added a new `apps/agents-server/src/middleware/contentSecurityPolicy.ts` module that generates a fresh per-request `nonce` and serializes a strict `Content-Security-Policy` header (`script-src 'nonce-…' 'strict-dynamic' …; object-src 'none'; base-uri 'self'`). Modern browsers therefore execute only inline scripts carrying the per-request nonce, while legacy browsers continue to work via the `'unsafe-inline'`/`https:`/`http:` fallback tokens that newer browsers ignore in the presence of a nonce.
    -   Wired the middleware in `apps/agents-server/src/middleware.ts` to issue the nonce, forward it on the request (`x-csp-nonce` plus a `Content-Security-Policy` request header so the Next.js runtime can pick it up for its own inline chunks), and set the response CSP header on every response.
    -   Updated `apps/agents-server/src/middleware/applyEmbeddingHeader.ts` to **append** its `frame-ancestors` directive instead of `set`, so the embed-chat route's framing policy no longer erases the base script-src policy.
    -   Rewrote `apps/agents-server/src/app/layout.tsx` to read the nonce via `headers()` and apply it to both inline `<script>` tags (theme-mode bootstrap and the aggregated custom JavaScript), and **removed the misleading `customJavascript.replace(/<\/script>/gi, '<\\/script>')` escape**. The custom JavaScript is now injected verbatim — it can only execute because of the nonce, and any closing-tag bypass attempts produced by an attacker would still be rejected by the browser CSP.
    -   Updated the **Custom JavaScript** admin page (`apps/agents-server/src/app/admin/custom-js/CustomJsClient.tsx`) to describe the new CSP-nonce trust model so admins know that runtime-created scripts inherit trust via `'strict-dynamic'` and that the closing-tag rewrite is no longer applied.
...
-   Changed `ptbk coder run` and `ptbk coder server` to run automatically through the prompt queue by default, replacing the previous `--no-wait` flag with a new `--no-auto` flag for opt-in interactive confirmation:
    ...
    -   Added `--no-auto` to wait for user confirmation before each prompt (the previous default behavior).
    -   Tightened `--wait` to require a duration value (e.g. `--wait 30m`); the previous "interactive mode" overload of `--wait` without a duration is replaced by `--no-auto`.
    -   Updated the `coder:run` script generated by `ptbk coder init`, the repository's own `package.json` `coder:run` script, the legacy `scripts/run-codex-prompts/cli/parseRunOptions.ts` arg parser, the `ptbk coder` help text and READMEs to use the new flag.
