...
-   Fixed Agents Server chat feedback stars so unpicked stars no longer stay yellow. The shared chat feedback controls now render CSS-colored star icons with their resolved color applied inline instead of the native colored emoji glyph, inactive stars are hollow, and the compact response control plus rating modal share the same rating-message key resolution so both reflect the picked rating consistently.
