...
-   Fixed the `ptbk coder run` rich terminal UI so command errors now expand detected runner script files into wrapped full-path `File` rows in the `Errors` panel. The file rows reuse the existing terminal hyperlink formatting, so long `.promptbook/coder-prompts/*.sh` paths remain visible and clickable even when the original error summary line is too long. The `ptbk coder` landing page live terminal sample now mirrors this `Errors` panel behavior.

-   Added the `ptbk coder add` command that appends one ready-to-run prompt file to the `prompts/` queue from a plain-language description. The description can be passed as an argument (`ptbk coder add "some new feature"`), piped in through stdin as a heredoc, or typed interactively when the command is run with no arguments; `--priority <n>` sets the new prompt's priority (rendered as trailing `!` markers) and `--template` reuses the same boilerplate templates as `ptbk coder generate-boilerplates`. Each added prompt receives a fresh unique emoji tag and the next sequential prompt number, and is written as a ready `[ ]` section so it runs without further editing. `ptbk coder init` now also creates a `coder:add` npm script, and the `ptbk coder` landing page quickstart demonstrates the command.

-   Fixed the `ptbk coder` landing page live terminal so the shared `AsciiOctopus` agent avatar renders as a crisp, solid creature matching the real `ptbk coder` terminal instead of a washed-out, faintly striped blob. The half-block (`▀` / `▄`) avatar rows are now drawn with a tight line-height so each block glyph fills its whole line box and the rows tile seamlessly like a real terminal. Only the landing display changed — the shared terminal avatar renderer, the CLI, and the horizontal dashboard centering are untouched.

-   Added local per-agent project folders to the Agents Server. A new `AgentProject` table links projects to `Agent.permanentId`, with each project backed by an isolated filesystem directory under `.promptbook/agents-server/projects` (overridable via `AGENT_PROJECTS_STORAGE_DIRECTORY`). Agent chat now gets local project tools for listing/creating projects, listing/reading/writing/deleting project files, running project scripts from inside the project folder, and returning authenticated project/file links that can be included in chat. Administrators can inspect all agent projects and their folders at `/admin/projects`, and `/admin/resource-monitor` now shows total project storage plus per-agent project sizes alongside the server disk-space metrics.

-   Added goal-driven autonomous scheduling to the Agents Server. Agents with a `GOAL` / `GOALS` commitment now automatically receive the existing timeout tools, including recurring `set_timeout` schedules via `recurrenceIntervalMs`, so a book like `GOAL Keep the Facebook page updated with new posts every day` is enough for the agent to create and maintain its own cron-like wake-ups. Creating an owned agent or updating its source now queues an idempotent goal-review turn in a stable `Agent goals` chat, and removing the goal cancels active agent-scoped wake-ups unless the source still explicitly contains `USE TIMEOUT`. The reused timeout manager is now reachable from the agent menu as **Goal Schedule** and labelled around wake-ups/recurring crons.
...
-   Changed standalone VPS Agents Server installs so an entered `OPENAI_API_KEY` configures the default OpenAI Codex code runner for API-key authentication and skips the initial interactive Codex login prompt.
    ...
    -   Fixed the OpenAI Codex runner shell to source the installed Agents Server `.env` through `PTBK_AGENTS_SERVER_ENV_FILE`, map `OPENAI_API_KEY` to `CODEX_API_KEY` for non-interactive `codex exec`, and pass `forced_login_method=api` when `PTBK_OPENAI_CODEX_USE_API_KEY=1`, preventing Codex from starting without the configured API key in local message runners.
