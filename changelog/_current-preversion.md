...
-   Added configurable agent identity for `scripts/run-codex-prompts`: commits now source their `user.name`/`user.email` from `CODING_AGENT_GIT_NAME`/`CODING_AGENT_GIT_EMAIL`, use `CODING_AGENT_GPG_KEY_ID` for signing, and a new `scripts/run-codex-prompts/README.md` plus `.env` defaults document how to swap in any other identity on a per-machine basis.
...
-   Ensured `scripts/run-codex-prompts` commits always run as the configurable coding agent identity (configured via `CODEX_AGENT_GIT_NAME`, `CODEX_AGENT_GIT_EMAIL`, and `CODEX_AGENT_GIT_SIGNING_KEY`) so commits are authored and GPG-signed by the agent instead of the primary user.
...
-   Added a metadata flag `IS_EMBEDDING_ALLOWED` (defaults to `true`) so administrators can gate the `/embed` iframe: when true the agent integrations page now shows an iframe snippet + live preview, and when false the embed route emits denying `Content-Security-Policy`/`X-Frame-Options` headers to block framing.
