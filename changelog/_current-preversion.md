-   Allowed duplicate agent names in Agents Server: removed the unique database constraint on `agentName` (via a new migration), updated `AgentCollectionInSupabase` and routing to resolve name collisions by prioritizing the oldest agent, removed duplicate-name blocks from the clone API, and added an in-editor diagnostic warning that underlines non-unique names in red without blocking saves.
