...
-   Added a new `specs/` documentation tree for Agents Server behavior, covering architecture, server routing, authentication, configuration, data model, agent lifecycle, book editing, chat runtime, durable user chats, management API, HTTP routes, user data, admin surfaces, integrations, database migrations, background workers, and UI navigation. The specs interlink the major responsibilities so the Agents Server functionality can be used as a single source of truth for development, testing, documentation, and compatible reimplementation.
...
-   Removed the placeholder **"Working on your request"** title and the **"What I'm Doing Now:"** wrapper from Agents Server durable chat progress cards so the response no longer shows generic headings between the `THINKING_MESSAGES` rotation and the final answer. The phase-based progress card (`createUserChatProgressCard`), the out-of-process runner progress card (`createUserChatRunnerProgressCard`), the in-process harness progress card (`createUserChatHarnessProgressCard`), and the `agent_progress` tool's default title fallback no longer set the literal `'Working on your request'` heading; the existing `now`/`next`/`items` real-state markdown still renders, with `now` shown directly instead of under a generic label, and the title field stays optional so agents can still supply their own meaningful heading through `agent_progress`.
...
-   Changed Agents Server durable chat progress so users see real progress in the requested sequence instead of an immediate synthetic queued card:
    ...
    -   When the local or external agent runner actually receives the request, the assistant placeholder now switches to a concise real runner-state card such as **"The local agent runner has the request and is working on the answer."**
